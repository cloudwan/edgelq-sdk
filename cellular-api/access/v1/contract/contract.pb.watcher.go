...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *contract.Contract_FieldMask
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[contract.Name]*contract.Contract
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*contract.ContractChange
}

func NewWatcher(client contract_client.ContractServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "contract-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[contract.Name]*contract.Contract{}
...
type WatcherEventChange struct {
	pre, post *contract.Contract
	name      *contract.Name
}

func NewAddWatcherEventChange(resource *contract.Contract) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *contract.Contract) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *contract.Contract) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *contract.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *contract.Contract {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *contract.Contract {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *contract.Contract {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *contract.Contract {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[contract.Name]*contract.Contract, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		contract := currentChange.GetCurrent()
		byName[*contract.GetName()] = contract
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			contract := change.GetCurrent()
			byName[*contract.GetName()] = contract
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, contract := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(contract))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[contract.Name]*contract.Contract{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *contract.Contract
		switch item := change.ChangeType.(type) {
		case *contract.ContractChange_Added_:
			currentItem = item.Added.GetContract()
		case *contract.ContractChange_Current_:
			currentItem = item.Current.GetContract()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*contract.ContractChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *contract.ContractChange_Added_:
			newItem := item.Added.GetContract()
		...
		case *contract.ContractChange_Modified_:
			updatedItem := item.Modified.GetContract()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*contract.ContractChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetContractName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(contract.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*contract.Contract_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*contract.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(contract_client.NewContractServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(contract.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*contract.Filter)
		}
		return params
	})
}
