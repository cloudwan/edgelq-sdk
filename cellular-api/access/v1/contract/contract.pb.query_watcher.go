...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*contract.ContractChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return contract.ContractChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client contract_client.ContractServiceClient,
...
func NewQueryWatcherWithIChan(id int, client contract_client.ContractServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "contract-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchContracts(ctx, &contract_client.WatchContractsRequest{
		...
		if err != nil {
			...
			pending := make([]*contract.ContractChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetContractChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetContractChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(contract.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(contract.ContractChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(contract.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*contract.PagerCursor)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*contract.Filter)
		}
		return NewQueryWatcherWithIChan(id, contract_client.NewContractServiceClient(cc), cfg, ch)
	})
}
