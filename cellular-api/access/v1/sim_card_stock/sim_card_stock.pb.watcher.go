...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *sim_card_stock.SimCardStock_FieldMask
	OrderBy   *sim_card_stock.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Filter *sim_card_stock.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[sim_card_stock.Name]*sim_card_stock.SimCardStock
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*sim_card_stock.SimCardStockChange
}

func NewWatcher(client sim_card_stock_client.SimCardStockServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Filter != nil {
			...
			cQuery.Filter = &sim_card_stock.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "simCardStock-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[sim_card_stock.Name]*sim_card_stock.SimCardStock{}
...
type WatcherEventChange struct {
	pre, post *sim_card_stock.SimCardStock
	name      *sim_card_stock.Name
}

func NewAddWatcherEventChange(resource *sim_card_stock.SimCardStock) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *sim_card_stock.SimCardStock) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *sim_card_stock.SimCardStock) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *sim_card_stock.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *sim_card_stock.SimCardStock {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *sim_card_stock.SimCardStock {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *sim_card_stock.SimCardStock {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *sim_card_stock.SimCardStock {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[sim_card_stock.Name]*sim_card_stock.SimCardStock, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		simCardStock := currentChange.GetCurrent()
		byName[*simCardStock.GetName()] = simCardStock
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			simCardStock := change.GetCurrent()
			byName[*simCardStock.GetName()] = simCardStock
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, simCardStock := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(simCardStock))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[sim_card_stock.Name]*sim_card_stock.SimCardStock{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *sim_card_stock.SimCardStock
		switch item := change.ChangeType.(type) {
		case *sim_card_stock.SimCardStockChange_Added_:
			currentItem = item.Added.GetSimCardStock()
		case *sim_card_stock.SimCardStockChange_Current_:
			currentItem = item.Current.GetSimCardStock()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*sim_card_stock.SimCardStockChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *sim_card_stock.SimCardStockChange_Added_:
			newItem := item.Added.GetSimCardStock()
		...
		case *sim_card_stock.SimCardStockChange_Modified_:
			updatedItem := item.Modified.GetSimCardStock()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*sim_card_stock.SimCardStockChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetSimCardStockName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(sim_card_stock.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*sim_card_stock.SimCardStock_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*sim_card_stock.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(sim_card_stock_client.NewSimCardStockServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(sim_card_stock.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*sim_card_stock.Filter)
		}
		return params
	})
}
