...
type QueryWatcherParams struct {
	...
	Cursor       *sim_card_stock.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*sim_card_stock.SimCardStockChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return sim_card_stock.SimCardStockChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client sim_card_stock_client.SimCardStockServiceClient,
...
func NewQueryWatcherWithIChan(id int, client sim_card_stock_client.SimCardStockServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "simCardStock-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchSimCardStocks(ctx, &sim_card_stock_client.WatchSimCardStocksRequest{
		...
		if err != nil {
			...
			pending := make([]*sim_card_stock.SimCardStockChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetSimCardStockChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetSimCardStockChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(sim_card_stock.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(sim_card_stock.SimCardStockChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(sim_card_stock.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*sim_card_stock.PagerCursor)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*sim_card_stock.Filter)
		}
		return NewQueryWatcherWithIChan(id, sim_card_stock_client.NewSimCardStockServiceClient(cc), cfg, ch)
	})
}
