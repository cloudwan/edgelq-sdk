...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*sim_card.SimCardChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return sim_card.SimCardChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client sim_card_client.SimCardServiceClient,
...
func NewQueryWatcherWithIChan(id int, client sim_card_client.SimCardServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "simCard-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchSimCards(ctx, &sim_card_client.WatchSimCardsRequest{
		...
		if err != nil {
			...
			pending := make([]*sim_card.SimCardChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetSimCardChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetSimCardChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(sim_card.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(sim_card.SimCardChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(sim_card.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*sim_card.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*sim_card.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*sim_card.Filter)
		}
		return NewQueryWatcherWithIChan(id, sim_card_client.NewSimCardServiceClient(cc), cfg, ch)
	})
}
