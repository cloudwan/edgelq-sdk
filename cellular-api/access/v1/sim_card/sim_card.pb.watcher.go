...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *sim_card.SimCard_FieldMask
	OrderBy   *sim_card.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *sim_card.ParentName
	Filter *sim_card.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[sim_card.Name]*sim_card.SimCard
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*sim_card.SimCardChange
}

func NewWatcher(client sim_card_client.SimCardServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &sim_card.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &sim_card.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "simCard-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[sim_card.Name]*sim_card.SimCard{}
...
type WatcherEventChange struct {
	pre, post *sim_card.SimCard
	name      *sim_card.Name
}

func NewAddWatcherEventChange(resource *sim_card.SimCard) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *sim_card.SimCard) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *sim_card.SimCard) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *sim_card.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *sim_card.SimCard {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *sim_card.SimCard {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *sim_card.SimCard {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *sim_card.SimCard {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[sim_card.Name]*sim_card.SimCard, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		simCard := currentChange.GetCurrent()
		byName[*simCard.GetName()] = simCard
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			simCard := change.GetCurrent()
			byName[*simCard.GetName()] = simCard
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, simCard := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(simCard))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[sim_card.Name]*sim_card.SimCard{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *sim_card.SimCard
		switch item := change.ChangeType.(type) {
		case *sim_card.SimCardChange_Added_:
			currentItem = item.Added.GetSimCard()
		case *sim_card.SimCardChange_Current_:
			currentItem = item.Current.GetSimCard()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*sim_card.SimCardChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *sim_card.SimCardChange_Added_:
			newItem := item.Added.GetSimCard()
		...
		case *sim_card.SimCardChange_Modified_:
			updatedItem := item.Modified.GetSimCard()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*sim_card.SimCardChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetSimCardName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(sim_card.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*sim_card.SimCard_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*sim_card.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(sim_card_client.NewSimCardServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(sim_card.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*sim_card.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*sim_card.ParentName)
		}
		return params
	})
}
