...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *conversation.Conversation_FieldMask
	OrderBy   *conversation.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *conversation.ParentName
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[conversation.Name]*conversation.Conversation
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*conversation.ConversationChange
}

func NewWatcher(client conversation_client.ConversationServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &conversation.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &conversation.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "conversation-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[conversation.Name]*conversation.Conversation{}
...
type WatcherEventChange struct {
	pre, post *conversation.Conversation
	name      *conversation.Name
}

func NewAddWatcherEventChange(resource *conversation.Conversation) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *conversation.Conversation) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *conversation.Conversation) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *conversation.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *conversation.Conversation {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *conversation.Conversation {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *conversation.Conversation {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *conversation.Conversation {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[conversation.Name]*conversation.Conversation, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		conversation := currentChange.GetCurrent()
		byName[*conversation.GetName()] = conversation
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			conversation := change.GetCurrent()
			byName[*conversation.GetName()] = conversation
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, conversation := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(conversation))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[conversation.Name]*conversation.Conversation{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *conversation.Conversation
		switch item := change.ChangeType.(type) {
		case *conversation.ConversationChange_Added_:
			currentItem = item.Added.GetConversation()
		case *conversation.ConversationChange_Current_:
			currentItem = item.Current.GetConversation()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*conversation.ConversationChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *conversation.ConversationChange_Added_:
			newItem := item.Added.GetConversation()
		...
		case *conversation.ConversationChange_Modified_:
			updatedItem := item.Modified.GetConversation()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*conversation.ConversationChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetConversationName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(conversation.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*conversation.Conversation_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*conversation.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(conversation_client.NewConversationServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(conversation.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*conversation.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*conversation.ParentName)
		}
		return params
	})
}
