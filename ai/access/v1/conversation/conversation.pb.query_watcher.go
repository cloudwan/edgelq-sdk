...
type QueryWatcherParams struct {
	...
	Cursor       *conversation.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*conversation.ConversationChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return conversation.ConversationChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client conversation_client.ConversationServiceClient,
...
func NewQueryWatcherWithIChan(id int, client conversation_client.ConversationServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "conversation-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchConversations(ctx, &conversation_client.WatchConversationsRequest{
		...
		if err != nil {
			...
			pending := make([]*conversation.ConversationChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetConversationChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetConversationChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(conversation.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(conversation.ConversationChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(conversation.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*conversation.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*conversation.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*conversation.Filter)
		}
		return NewQueryWatcherWithIChan(id, conversation_client.NewConversationServiceClient(cc), cfg, ch)
	})
}
