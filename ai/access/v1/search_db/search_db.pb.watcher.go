...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *search_db.SearchDb_FieldMask
	OrderBy   *search_db.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *search_db.ParentName
	Filter *search_db.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[search_db.Name]*search_db.SearchDb
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*search_db.SearchDbChange
}

func NewWatcher(client search_db_client.SearchDbServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &search_db.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &search_db.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "searchDb-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[search_db.Name]*search_db.SearchDb{}
...
type WatcherEventChange struct {
	pre, post *search_db.SearchDb
	name      *search_db.Name
}

func NewAddWatcherEventChange(resource *search_db.SearchDb) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *search_db.SearchDb) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *search_db.SearchDb) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *search_db.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *search_db.SearchDb {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *search_db.SearchDb {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *search_db.SearchDb {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *search_db.SearchDb {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[search_db.Name]*search_db.SearchDb, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		searchDb := currentChange.GetCurrent()
		byName[*searchDb.GetName()] = searchDb
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			searchDb := change.GetCurrent()
			byName[*searchDb.GetName()] = searchDb
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, searchDb := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(searchDb))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[search_db.Name]*search_db.SearchDb{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *search_db.SearchDb
		switch item := change.ChangeType.(type) {
		case *search_db.SearchDbChange_Added_:
			currentItem = item.Added.GetSearchDb()
		case *search_db.SearchDbChange_Current_:
			currentItem = item.Current.GetSearchDb()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*search_db.SearchDbChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *search_db.SearchDbChange_Added_:
			newItem := item.Added.GetSearchDb()
		...
		case *search_db.SearchDbChange_Modified_:
			updatedItem := item.Modified.GetSearchDb()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*search_db.SearchDbChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetSearchDbName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(search_db.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*search_db.SearchDb_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*search_db.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(search_db_client.NewSearchDbServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(search_db.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*search_db.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*search_db.ParentName)
		}
		return params
	})
}
