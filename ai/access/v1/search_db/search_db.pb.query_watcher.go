...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*search_db.SearchDbChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return search_db.SearchDbChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client search_db_client.SearchDbServiceClient,
...
func NewQueryWatcherWithIChan(id int, client search_db_client.SearchDbServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "searchDb-query-watcher").
	...
	for {
		stream, err := qw.client.WatchSearchDbs(ctx, &search_db_client.WatchSearchDbsRequest{
		...
		if err != nil {
			...
			pending := make([]*search_db.SearchDbChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetSearchDbChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetSearchDbChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(search_db.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(search_db.SearchDbChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(search_db.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*search_db.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*search_db.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*search_db.Filter)
		}
		return NewQueryWatcherWithIChan(id, search_db_client.NewSearchDbServiceClient(cc), cfg, ch)
	})
}
