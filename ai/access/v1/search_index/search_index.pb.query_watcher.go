...
type QueryWatcherParams struct {
	...
	Cursor       *search_index.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*search_index.SearchIndexChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return search_index.SearchIndexChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client search_index_client.SearchIndexServiceClient,
...
func NewQueryWatcherWithIChan(id int, client search_index_client.SearchIndexServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "searchIndex-query-watcher").
	...
	for {
		stream, err := qw.client.WatchSearchIndices(ctx, &search_index_client.WatchSearchIndicesRequest{
		...
		if err != nil {
			...
			pending := make([]*search_index.SearchIndexChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetSearchIndexChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetSearchIndexChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(search_index.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(search_index.SearchIndexChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(search_index.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*search_index.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*search_index.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*search_index.Filter)
		}
		return NewQueryWatcherWithIChan(id, search_index_client.NewSearchIndexServiceClient(cc), cfg, ch)
	})
}
