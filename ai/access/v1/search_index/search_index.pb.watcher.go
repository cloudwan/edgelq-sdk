...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *search_index.SearchIndex_FieldMask
	OrderBy   *search_index.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *search_index.ParentName
	Filter *search_index.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[search_index.Name]*search_index.SearchIndex
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*search_index.SearchIndexChange
}

func NewWatcher(client search_index_client.SearchIndexServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &search_index.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &search_index.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "searchIndex-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[search_index.Name]*search_index.SearchIndex{}
...
type WatcherEventChange struct {
	pre, post *search_index.SearchIndex
	name      *search_index.Name
}

func NewAddWatcherEventChange(resource *search_index.SearchIndex) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *search_index.SearchIndex) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *search_index.SearchIndex) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *search_index.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *search_index.SearchIndex {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *search_index.SearchIndex {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *search_index.SearchIndex {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *search_index.SearchIndex {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[search_index.Name]*search_index.SearchIndex, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		searchIndex := currentChange.GetCurrent()
		byName[*searchIndex.GetName()] = searchIndex
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			searchIndex := change.GetCurrent()
			byName[*searchIndex.GetName()] = searchIndex
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, searchIndex := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(searchIndex))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[search_index.Name]*search_index.SearchIndex{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *search_index.SearchIndex
		switch item := change.ChangeType.(type) {
		case *search_index.SearchIndexChange_Added_:
			currentItem = item.Added.GetSearchIndex()
		case *search_index.SearchIndexChange_Current_:
			currentItem = item.Current.GetSearchIndex()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*search_index.SearchIndexChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *search_index.SearchIndexChange_Added_:
			newItem := item.Added.GetSearchIndex()
		...
		case *search_index.SearchIndexChange_Modified_:
			updatedItem := item.Modified.GetSearchIndex()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*search_index.SearchIndexChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetSearchIndexName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(search_index.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*search_index.SearchIndex_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*search_index.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(search_index_client.NewSearchIndexServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(search_index.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*search_index.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*search_index.ParentName)
		}
		return params
	})
}
