...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *conversation_feedback.ConversationFeedback_FieldMask
	OrderBy   *conversation_feedback.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *conversation_feedback.ParentName
	Filter *conversation_feedback.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[conversation_feedback.Name]*conversation_feedback.ConversationFeedback
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*conversation_feedback.ConversationFeedbackChange
}

func NewWatcher(client conversation_feedback_client.ConversationFeedbackServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &conversation_feedback.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &conversation_feedback.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "conversationFeedback-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[conversation_feedback.Name]*conversation_feedback.ConversationFeedback{}
...
type WatcherEventChange struct {
	pre, post *conversation_feedback.ConversationFeedback
	name      *conversation_feedback.Name
}

func NewAddWatcherEventChange(resource *conversation_feedback.ConversationFeedback) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *conversation_feedback.ConversationFeedback) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *conversation_feedback.ConversationFeedback) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *conversation_feedback.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *conversation_feedback.ConversationFeedback {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *conversation_feedback.ConversationFeedback {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *conversation_feedback.ConversationFeedback {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *conversation_feedback.ConversationFeedback {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[conversation_feedback.Name]*conversation_feedback.ConversationFeedback, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		conversationFeedback := currentChange.GetCurrent()
		byName[*conversationFeedback.GetName()] = conversationFeedback
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			conversationFeedback := change.GetCurrent()
			byName[*conversationFeedback.GetName()] = conversationFeedback
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, conversationFeedback := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(conversationFeedback))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[conversation_feedback.Name]*conversation_feedback.ConversationFeedback{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *conversation_feedback.ConversationFeedback
		switch item := change.ChangeType.(type) {
		case *conversation_feedback.ConversationFeedbackChange_Added_:
			currentItem = item.Added.GetConversationFeedback()
		case *conversation_feedback.ConversationFeedbackChange_Current_:
			currentItem = item.Current.GetConversationFeedback()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*conversation_feedback.ConversationFeedbackChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *conversation_feedback.ConversationFeedbackChange_Added_:
			newItem := item.Added.GetConversationFeedback()
		...
		case *conversation_feedback.ConversationFeedbackChange_Modified_:
			updatedItem := item.Modified.GetConversationFeedback()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*conversation_feedback.ConversationFeedbackChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetConversationFeedbackName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(conversation_feedback.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*conversation_feedback.ConversationFeedback_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*conversation_feedback.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(conversation_feedback_client.NewConversationFeedbackServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(conversation_feedback.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*conversation_feedback.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*conversation_feedback.ParentName)
		}
		return params
	})
}
