...
type QueryWatcherParams struct {
	...
	Cursor       *conversation_feedback.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*conversation_feedback.ConversationFeedbackChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return conversation_feedback.ConversationFeedbackChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client conversation_feedback_client.ConversationFeedbackServiceClient,
...
func NewQueryWatcherWithIChan(id int, client conversation_feedback_client.ConversationFeedbackServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "conversationFeedback-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchConversationFeedbacks(ctx, &conversation_feedback_client.WatchConversationFeedbacksRequest{
		...
		if err != nil {
			...
			pending := make([]*conversation_feedback.ConversationFeedbackChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetConversationFeedbackChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetConversationFeedbackChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(conversation_feedback.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(conversation_feedback.ConversationFeedbackChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(conversation_feedback.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*conversation_feedback.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*conversation_feedback.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*conversation_feedback.Filter)
		}
		return NewQueryWatcherWithIChan(id, conversation_feedback_client.NewConversationFeedbackServiceClient(cc), cfg, ch)
	})
}
