...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *document.Document_FieldMask
	OrderBy   *document.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *document.ParentName
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[document.Name]*document.Document
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*document.DocumentChange
}

func NewWatcher(client document_client.DocumentServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &document.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &document.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "document-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[document.Name]*document.Document{}
...
type WatcherEventChange struct {
	pre, post *document.Document
	name      *document.Name
}

func NewAddWatcherEventChange(resource *document.Document) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *document.Document) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *document.Document) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *document.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *document.Document {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *document.Document {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *document.Document {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *document.Document {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[document.Name]*document.Document, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		document := currentChange.GetCurrent()
		byName[*document.GetName()] = document
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			document := change.GetCurrent()
			byName[*document.GetName()] = document
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, document := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(document))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[document.Name]*document.Document{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *document.Document
		switch item := change.ChangeType.(type) {
		case *document.DocumentChange_Added_:
			currentItem = item.Added.GetDocument()
		case *document.DocumentChange_Current_:
			currentItem = item.Current.GetDocument()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*document.DocumentChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *document.DocumentChange_Added_:
			newItem := item.Added.GetDocument()
		...
		case *document.DocumentChange_Modified_:
			updatedItem := item.Modified.GetDocument()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*document.DocumentChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetDocumentName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(document.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*document.Document_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*document.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(document_client.NewDocumentServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(document.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*document.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*document.ParentName)
		}
		return params
	})
}
