...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*document.DocumentChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return document.DocumentChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client document_client.DocumentServiceClient,
...
func NewQueryWatcherWithIChan(id int, client document_client.DocumentServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "document-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchDocuments(ctx, &document_client.WatchDocumentsRequest{
		...
		if err != nil {
			...
			pending := make([]*document.DocumentChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetDocumentChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetDocumentChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(document.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(document.DocumentChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(document.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*document.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*document.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*document.Filter)
		}
		return NewQueryWatcherWithIChan(id, document_client.NewDocumentServiceClient(cc), cfg, ch)
	})
}
