...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *connector.Connector_FieldMask
	OrderBy   *connector.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *connector.ParentName
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[connector.Name]*connector.Connector
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*connector.ConnectorChange
}

func NewWatcher(client connector_client.ConnectorServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &connector.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &connector.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "connector-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[connector.Name]*connector.Connector{}
...
type WatcherEventChange struct {
	pre, post *connector.Connector
	name      *connector.Name
}

func NewAddWatcherEventChange(resource *connector.Connector) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *connector.Connector) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *connector.Connector) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *connector.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *connector.Connector {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *connector.Connector {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *connector.Connector {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *connector.Connector {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[connector.Name]*connector.Connector, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		connector := currentChange.GetCurrent()
		byName[*connector.GetName()] = connector
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			connector := change.GetCurrent()
			byName[*connector.GetName()] = connector
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, connector := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(connector))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[connector.Name]*connector.Connector{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *connector.Connector
		switch item := change.ChangeType.(type) {
		case *connector.ConnectorChange_Added_:
			currentItem = item.Added.GetConnector()
		case *connector.ConnectorChange_Current_:
			currentItem = item.Current.GetConnector()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*connector.ConnectorChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *connector.ConnectorChange_Added_:
			newItem := item.Added.GetConnector()
		...
		case *connector.ConnectorChange_Modified_:
			updatedItem := item.Modified.GetConnector()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*connector.ConnectorChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetConnectorName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(connector.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*connector.Connector_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*connector.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(connector_client.NewConnectorServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(connector.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*connector.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*connector.ParentName)
		}
		return params
	})
}
