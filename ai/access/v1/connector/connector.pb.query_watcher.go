...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*connector.ConnectorChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return connector.ConnectorChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client connector_client.ConnectorServiceClient,
...
func NewQueryWatcherWithIChan(id int, client connector_client.ConnectorServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "connector-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchConnectors(ctx, &connector_client.WatchConnectorsRequest{
		...
		if err != nil {
			...
			pending := make([]*connector.ConnectorChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetConnectorChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetConnectorChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(connector.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(connector.ConnectorChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(connector.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*connector.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*connector.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*connector.Filter)
		}
		return NewQueryWatcherWithIChan(id, connector_client.NewConnectorServiceClient(cc), cfg, ch)
	})
}
