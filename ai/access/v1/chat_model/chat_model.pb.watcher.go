...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *chat_model.ChatModel_FieldMask
	OrderBy   *chat_model.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *chat_model.ParentName
	Filter *chat_model.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[chat_model.Name]*chat_model.ChatModel
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*chat_model.ChatModelChange
}

func NewWatcher(client chat_model_client.ChatModelServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &chat_model.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &chat_model.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "chatModel-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[chat_model.Name]*chat_model.ChatModel{}
...
type WatcherEventChange struct {
	pre, post *chat_model.ChatModel
	name      *chat_model.Name
}

func NewAddWatcherEventChange(resource *chat_model.ChatModel) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *chat_model.ChatModel) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *chat_model.ChatModel) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *chat_model.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *chat_model.ChatModel {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *chat_model.ChatModel {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *chat_model.ChatModel {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *chat_model.ChatModel {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[chat_model.Name]*chat_model.ChatModel, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		chatModel := currentChange.GetCurrent()
		byName[*chatModel.GetName()] = chatModel
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			chatModel := change.GetCurrent()
			byName[*chatModel.GetName()] = chatModel
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, chatModel := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(chatModel))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[chat_model.Name]*chat_model.ChatModel{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *chat_model.ChatModel
		switch item := change.ChangeType.(type) {
		case *chat_model.ChatModelChange_Added_:
			currentItem = item.Added.GetChatModel()
		case *chat_model.ChatModelChange_Current_:
			currentItem = item.Current.GetChatModel()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*chat_model.ChatModelChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *chat_model.ChatModelChange_Added_:
			newItem := item.Added.GetChatModel()
		...
		case *chat_model.ChatModelChange_Modified_:
			updatedItem := item.Modified.GetChatModel()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*chat_model.ChatModelChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetChatModelName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(chat_model.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*chat_model.ChatModel_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*chat_model.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(chat_model_client.NewChatModelServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(chat_model.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*chat_model.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*chat_model.ParentName)
		}
		return params
	})
}
