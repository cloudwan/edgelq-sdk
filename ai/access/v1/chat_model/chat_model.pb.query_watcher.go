...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*chat_model.ChatModelChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return chat_model.ChatModelChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client chat_model_client.ChatModelServiceClient,
...
func NewQueryWatcherWithIChan(id int, client chat_model_client.ChatModelServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "chatModel-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchChatModels(ctx, &chat_model_client.WatchChatModelsRequest{
		...
		if err != nil {
			...
			pending := make([]*chat_model.ChatModelChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetChatModelChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetChatModelChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(chat_model.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(chat_model.ChatModelChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(chat_model.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*chat_model.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*chat_model.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*chat_model.Filter)
		}
		return NewQueryWatcherWithIChan(id, chat_model_client.NewChatModelServiceClient(cc), cfg, ch)
	})
}
