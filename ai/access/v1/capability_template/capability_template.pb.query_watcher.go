...
type QueryWatcherParams struct {
	...
	Cursor       *capability_template.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*capability_template.CapabilityTemplateChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return capability_template.CapabilityTemplateChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client capability_template_client.CapabilityTemplateServiceClient,
...
func NewQueryWatcherWithIChan(id int, client capability_template_client.CapabilityTemplateServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "capabilityTemplate-query-watcher").
	...
	for {
		stream, err := qw.client.WatchCapabilityTemplates(ctx, &capability_template_client.WatchCapabilityTemplatesRequest{
		...
		if err != nil {
			...
			pending := make([]*capability_template.CapabilityTemplateChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetCapabilityTemplateChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetCapabilityTemplateChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(capability_template.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(capability_template.CapabilityTemplateChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(capability_template.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*capability_template.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*capability_template.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*capability_template.Filter)
		}
		return NewQueryWatcherWithIChan(id, capability_template_client.NewCapabilityTemplateServiceClient(cc), cfg, ch)
	})
}
