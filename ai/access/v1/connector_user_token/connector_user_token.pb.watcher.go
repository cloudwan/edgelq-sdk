...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *connector_user_token.ConnectorUserToken_FieldMask
	OrderBy   *connector_user_token.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *connector_user_token.ParentName
	Filter *connector_user_token.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[connector_user_token.Name]*connector_user_token.ConnectorUserToken
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*connector_user_token.ConnectorUserTokenChange
}

func NewWatcher(client connector_user_token_client.ConnectorUserTokenServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &connector_user_token.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &connector_user_token.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "connectorUserToken-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[connector_user_token.Name]*connector_user_token.ConnectorUserToken{}
...
type WatcherEventChange struct {
	pre, post *connector_user_token.ConnectorUserToken
	name      *connector_user_token.Name
}

func NewAddWatcherEventChange(resource *connector_user_token.ConnectorUserToken) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *connector_user_token.ConnectorUserToken) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *connector_user_token.ConnectorUserToken) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *connector_user_token.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *connector_user_token.ConnectorUserToken {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *connector_user_token.ConnectorUserToken {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *connector_user_token.ConnectorUserToken {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *connector_user_token.ConnectorUserToken {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[connector_user_token.Name]*connector_user_token.ConnectorUserToken, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		connectorUserToken := currentChange.GetCurrent()
		byName[*connectorUserToken.GetName()] = connectorUserToken
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			connectorUserToken := change.GetCurrent()
			byName[*connectorUserToken.GetName()] = connectorUserToken
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, connectorUserToken := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(connectorUserToken))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[connector_user_token.Name]*connector_user_token.ConnectorUserToken{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *connector_user_token.ConnectorUserToken
		switch item := change.ChangeType.(type) {
		case *connector_user_token.ConnectorUserTokenChange_Added_:
			currentItem = item.Added.GetConnectorUserToken()
		case *connector_user_token.ConnectorUserTokenChange_Current_:
			currentItem = item.Current.GetConnectorUserToken()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*connector_user_token.ConnectorUserTokenChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *connector_user_token.ConnectorUserTokenChange_Added_:
			newItem := item.Added.GetConnectorUserToken()
		...
		case *connector_user_token.ConnectorUserTokenChange_Modified_:
			updatedItem := item.Modified.GetConnectorUserToken()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*connector_user_token.ConnectorUserTokenChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetConnectorUserTokenName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(connector_user_token.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*connector_user_token.ConnectorUserToken_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*connector_user_token.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(connector_user_token_client.NewConnectorUserTokenServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(connector_user_token.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*connector_user_token.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*connector_user_token.ParentName)
		}
		return params
	})
}
