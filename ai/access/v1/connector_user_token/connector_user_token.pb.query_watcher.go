...
type QueryWatcherParams struct {
	...
	Cursor       *connector_user_token.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*connector_user_token.ConnectorUserTokenChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return connector_user_token.ConnectorUserTokenChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client connector_user_token_client.ConnectorUserTokenServiceClient,
...
func NewQueryWatcherWithIChan(id int, client connector_user_token_client.ConnectorUserTokenServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "connectorUserToken-query-watcher").
	...
	for {
		stream, err := qw.client.WatchConnectorUserTokens(ctx, &connector_user_token_client.WatchConnectorUserTokensRequest{
		...
		if err != nil {
			...
			pending := make([]*connector_user_token.ConnectorUserTokenChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetConnectorUserTokenChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetConnectorUserTokenChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(connector_user_token.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(connector_user_token.ConnectorUserTokenChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(connector_user_token.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*connector_user_token.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*connector_user_token.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*connector_user_token.Filter)
		}
		return NewQueryWatcherWithIChan(id, connector_user_token_client.NewConnectorUserTokenServiceClient(cc), cfg, ch)
	})
}
