...
type QueryWatcherParams struct {
	...
	Cursor       *edgelq_instance.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*edgelq_instance.EdgelqInstanceChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return edgelq_instance.EdgelqInstanceChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client edgelq_instance_client.EdgelqInstanceServiceClient,
...
func NewQueryWatcherWithIChan(id int, client edgelq_instance_client.EdgelqInstanceServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "edgelqInstance-query-watcher").
	...
	for {
		stream, err := qw.client.WatchEdgelqInstances(ctx, &edgelq_instance_client.WatchEdgelqInstancesRequest{
		...
		if err != nil {
			...
			pending := make([]*edgelq_instance.EdgelqInstanceChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetEdgelqInstanceChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetEdgelqInstanceChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(edgelq_instance.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(edgelq_instance.EdgelqInstanceChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(edgelq_instance.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*edgelq_instance.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*edgelq_instance.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*edgelq_instance.Filter)
		}
		return NewQueryWatcherWithIChan(id, edgelq_instance_client.NewEdgelqInstanceServiceClient(cc), cfg, ch)
	})
}
