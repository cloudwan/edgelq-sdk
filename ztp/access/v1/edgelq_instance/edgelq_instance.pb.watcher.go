...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *edgelq_instance.EdgelqInstance_FieldMask
	OrderBy   *edgelq_instance.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *edgelq_instance.ParentName
	Filter *edgelq_instance.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[edgelq_instance.Name]*edgelq_instance.EdgelqInstance
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*edgelq_instance.EdgelqInstanceChange
}

func NewWatcher(client edgelq_instance_client.EdgelqInstanceServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &edgelq_instance.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &edgelq_instance.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "edgelqInstance-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[edgelq_instance.Name]*edgelq_instance.EdgelqInstance{}
...
type WatcherEventChange struct {
	pre, post *edgelq_instance.EdgelqInstance
	name      *edgelq_instance.Name
}

func NewAddWatcherEventChange(resource *edgelq_instance.EdgelqInstance) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *edgelq_instance.EdgelqInstance) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *edgelq_instance.EdgelqInstance) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *edgelq_instance.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *edgelq_instance.EdgelqInstance {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *edgelq_instance.EdgelqInstance {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *edgelq_instance.EdgelqInstance {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *edgelq_instance.EdgelqInstance {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[edgelq_instance.Name]*edgelq_instance.EdgelqInstance, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		edgelqInstance := currentChange.GetCurrent()
		byName[*edgelqInstance.GetName()] = edgelqInstance
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			edgelqInstance := change.GetCurrent()
			byName[*edgelqInstance.GetName()] = edgelqInstance
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, edgelqInstance := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(edgelqInstance))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[edgelq_instance.Name]*edgelq_instance.EdgelqInstance{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *edgelq_instance.EdgelqInstance
		switch item := change.ChangeType.(type) {
		case *edgelq_instance.EdgelqInstanceChange_Added_:
			currentItem = item.Added.GetEdgelqInstance()
		case *edgelq_instance.EdgelqInstanceChange_Current_:
			currentItem = item.Current.GetEdgelqInstance()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*edgelq_instance.EdgelqInstanceChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *edgelq_instance.EdgelqInstanceChange_Added_:
			newItem := item.Added.GetEdgelqInstance()
		...
		case *edgelq_instance.EdgelqInstanceChange_Modified_:
			updatedItem := item.Modified.GetEdgelqInstance()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*edgelq_instance.EdgelqInstanceChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetEdgelqInstanceName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(edgelq_instance.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*edgelq_instance.EdgelqInstance_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*edgelq_instance.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(edgelq_instance_client.NewEdgelqInstanceServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(edgelq_instance.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*edgelq_instance.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*edgelq_instance.ParentName)
		}
		return params
	})
}
