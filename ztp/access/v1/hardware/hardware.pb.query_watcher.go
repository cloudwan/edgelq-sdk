...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*hardware.HardwareChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return hardware.HardwareChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client hardware_client.HardwareServiceClient,
...
func NewQueryWatcherWithIChan(id int, client hardware_client.HardwareServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "hardware-query-watcher").
	...
	for {
		stream, err := qw.client.WatchHardwares(ctx, &hardware_client.WatchHardwaresRequest{
		...
		if err != nil {
			...
			pending := make([]*hardware.HardwareChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetHardwareChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetHardwareChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(hardware.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(hardware.HardwareChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(hardware.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*hardware.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*hardware.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*hardware.Filter)
		}
		return NewQueryWatcherWithIChan(id, hardware_client.NewHardwareServiceClient(cc), cfg, ch)
	})
}
