...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *hardware.Hardware_FieldMask
	OrderBy   *hardware.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *hardware.ParentName
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[hardware.Name]*hardware.Hardware
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*hardware.HardwareChange
}

func NewWatcher(client hardware_client.HardwareServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &hardware.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &hardware.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "hardware-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[hardware.Name]*hardware.Hardware{}
...
type WatcherEventChange struct {
	pre, post *hardware.Hardware
	name      *hardware.Name
}

func NewAddWatcherEventChange(resource *hardware.Hardware) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *hardware.Hardware) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *hardware.Hardware) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *hardware.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *hardware.Hardware {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *hardware.Hardware {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *hardware.Hardware {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *hardware.Hardware {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[hardware.Name]*hardware.Hardware, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		hardware := currentChange.GetCurrent()
		byName[*hardware.GetName()] = hardware
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			hardware := change.GetCurrent()
			byName[*hardware.GetName()] = hardware
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, hardware := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(hardware))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[hardware.Name]*hardware.Hardware{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *hardware.Hardware
		switch item := change.ChangeType.(type) {
		case *hardware.HardwareChange_Added_:
			currentItem = item.Added.GetHardware()
		case *hardware.HardwareChange_Current_:
			currentItem = item.Current.GetHardware()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*hardware.HardwareChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *hardware.HardwareChange_Added_:
			newItem := item.Added.GetHardware()
		...
		case *hardware.HardwareChange_Modified_:
			updatedItem := item.Modified.GetHardware()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*hardware.HardwareChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetHardwareName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(hardware.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*hardware.Hardware_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*hardware.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(hardware_client.NewHardwareServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(hardware.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*hardware.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*hardware.ParentName)
		}
		return params
	})
}
