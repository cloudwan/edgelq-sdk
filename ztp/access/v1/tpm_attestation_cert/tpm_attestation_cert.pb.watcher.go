...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *tpm_attestation_cert.TpmAttestationCert_FieldMask
	OrderBy   *tpm_attestation_cert.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *tpm_attestation_cert.ParentName
	Filter *tpm_attestation_cert.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[tpm_attestation_cert.Name]*tpm_attestation_cert.TpmAttestationCert
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*tpm_attestation_cert.TpmAttestationCertChange
}

func NewWatcher(client tpm_attestation_cert_client.TpmAttestationCertServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &tpm_attestation_cert.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &tpm_attestation_cert.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "tpmAttestationCert-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[tpm_attestation_cert.Name]*tpm_attestation_cert.TpmAttestationCert{}
...
type WatcherEventChange struct {
	pre, post *tpm_attestation_cert.TpmAttestationCert
	name      *tpm_attestation_cert.Name
}

func NewAddWatcherEventChange(resource *tpm_attestation_cert.TpmAttestationCert) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *tpm_attestation_cert.TpmAttestationCert) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *tpm_attestation_cert.TpmAttestationCert) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *tpm_attestation_cert.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *tpm_attestation_cert.TpmAttestationCert {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *tpm_attestation_cert.TpmAttestationCert {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *tpm_attestation_cert.TpmAttestationCert {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *tpm_attestation_cert.TpmAttestationCert {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[tpm_attestation_cert.Name]*tpm_attestation_cert.TpmAttestationCert, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		tpmAttestationCert := currentChange.GetCurrent()
		byName[*tpmAttestationCert.GetName()] = tpmAttestationCert
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			tpmAttestationCert := change.GetCurrent()
			byName[*tpmAttestationCert.GetName()] = tpmAttestationCert
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, tpmAttestationCert := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(tpmAttestationCert))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[tpm_attestation_cert.Name]*tpm_attestation_cert.TpmAttestationCert{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *tpm_attestation_cert.TpmAttestationCert
		switch item := change.ChangeType.(type) {
		case *tpm_attestation_cert.TpmAttestationCertChange_Added_:
			currentItem = item.Added.GetTpmAttestationCert()
		case *tpm_attestation_cert.TpmAttestationCertChange_Current_:
			currentItem = item.Current.GetTpmAttestationCert()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*tpm_attestation_cert.TpmAttestationCertChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *tpm_attestation_cert.TpmAttestationCertChange_Added_:
			newItem := item.Added.GetTpmAttestationCert()
		...
		case *tpm_attestation_cert.TpmAttestationCertChange_Modified_:
			updatedItem := item.Modified.GetTpmAttestationCert()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*tpm_attestation_cert.TpmAttestationCertChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetTpmAttestationCertName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(tpm_attestation_cert.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*tpm_attestation_cert.TpmAttestationCert_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*tpm_attestation_cert.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(tpm_attestation_cert_client.NewTpmAttestationCertServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(tpm_attestation_cert.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*tpm_attestation_cert.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*tpm_attestation_cert.ParentName)
		}
		return params
	})
}
