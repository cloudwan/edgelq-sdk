...
func (obj *PlanAssignment) GotenValidate() error {
	...
	for idx, elem := range obj.RegionalPlanOverrides {
		if subobj, ok := interface{}(elem).(gotenvalidate.Validator); ok {
			if err := subobj.GotenValidate(); err != nil {
				return gotenvalidate.NewValidationError("PlanAssignment", "regionalPlanOverrides", obj.RegionalPlanOverrides[idx], "nested object validation failed", err)
			}
		}
	}
	...
	for idx, elem := range obj.Allowances {
		if subobj, ok := interface{}(elem).(gotenvalidate.Validator); ok {
			if err := subobj.GotenValidate(); err != nil {
				return gotenvalidate.NewValidationError("PlanAssignment", "allowances", obj.Allowances[idx], "nested object validation failed", err)
			}
		}
	}
