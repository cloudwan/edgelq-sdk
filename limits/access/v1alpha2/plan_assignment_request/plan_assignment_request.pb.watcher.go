...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *plan_assignment_request.PlanAssignmentRequest_FieldMask
	OrderBy   *plan_assignment_request.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *plan_assignment_request.ParentReference
	Filter *plan_assignment_request.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[plan_assignment_request.Name]*plan_assignment_request.PlanAssignmentRequest
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*plan_assignment_request.PlanAssignmentRequestChange
}

func NewWatcher(client plan_assignment_request_client.PlanAssignmentRequestServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &plan_assignment_request.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &plan_assignment_request.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "planAssignmentRequest-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[plan_assignment_request.Name]*plan_assignment_request.PlanAssignmentRequest{}
...
type WatcherEventChange struct {
	pre, post *plan_assignment_request.PlanAssignmentRequest
	name      *plan_assignment_request.Name
}

func NewAddWatcherEventChange(resource *plan_assignment_request.PlanAssignmentRequest) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *plan_assignment_request.PlanAssignmentRequest) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *plan_assignment_request.PlanAssignmentRequest) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *plan_assignment_request.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *plan_assignment_request.PlanAssignmentRequest {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *plan_assignment_request.PlanAssignmentRequest {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *plan_assignment_request.PlanAssignmentRequest {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *plan_assignment_request.PlanAssignmentRequest {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[plan_assignment_request.Name]*plan_assignment_request.PlanAssignmentRequest, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		planAssignmentRequest := currentChange.GetCurrent()
		byName[*planAssignmentRequest.GetName()] = planAssignmentRequest
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			planAssignmentRequest := change.GetCurrent()
			byName[*planAssignmentRequest.GetName()] = planAssignmentRequest
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, planAssignmentRequest := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(planAssignmentRequest))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[plan_assignment_request.Name]*plan_assignment_request.PlanAssignmentRequest{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *plan_assignment_request.PlanAssignmentRequest
		switch item := change.ChangeType.(type) {
		case *plan_assignment_request.PlanAssignmentRequestChange_Added_:
			currentItem = item.Added.GetPlanAssignmentRequest()
		case *plan_assignment_request.PlanAssignmentRequestChange_Current_:
			currentItem = item.Current.GetPlanAssignmentRequest()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*plan_assignment_request.PlanAssignmentRequestChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *plan_assignment_request.PlanAssignmentRequestChange_Added_:
			newItem := item.Added.GetPlanAssignmentRequest()
		...
		case *plan_assignment_request.PlanAssignmentRequestChange_Modified_:
			updatedItem := item.Modified.GetPlanAssignmentRequest()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*plan_assignment_request.PlanAssignmentRequestChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetPlanAssignmentRequestName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(plan_assignment_request.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*plan_assignment_request.PlanAssignmentRequest_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*plan_assignment_request.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(plan_assignment_request_client.NewPlanAssignmentRequestServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(plan_assignment_request.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*plan_assignment_request.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*plan_assignment_request.ParentReference)
		}
		return params
	})
}
