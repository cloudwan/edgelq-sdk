...
type WatcherFilterParams struct {
	Parent *plan_assignment_request.ParentName
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &plan_assignment_request.ParentName{}
...
func init() {
	...
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(plan_assignment_request.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*plan_assignment_request.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*plan_assignment_request.ParentName)
		}
		return params
	})
}
