...
type QueryWatcherParams struct {
	...
	Cursor       *plan_assignment_request.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*plan_assignment_request.PlanAssignmentRequestChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return plan_assignment_request.PlanAssignmentRequestChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client plan_assignment_request_client.PlanAssignmentRequestServiceClient,
...
func NewQueryWatcherWithIChan(id int, client plan_assignment_request_client.PlanAssignmentRequestServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "planAssignmentRequest-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchPlanAssignmentRequests(ctx, &plan_assignment_request_client.WatchPlanAssignmentRequestsRequest{
		...
		if err != nil {
			...
			pending := make([]*plan_assignment_request.PlanAssignmentRequestChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetPlanAssignmentRequestChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetPlanAssignmentRequestChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(plan_assignment_request.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(plan_assignment_request.PlanAssignmentRequestChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(plan_assignment_request.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*plan_assignment_request.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*plan_assignment_request.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*plan_assignment_request.Filter)
		}
		return NewQueryWatcherWithIChan(id, plan_assignment_request_client.NewPlanAssignmentRequestServiceClient(cc), cfg, ch)
	})
}
