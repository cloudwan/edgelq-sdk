...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *plan_assignment.PlanAssignment_FieldMask
	OrderBy   *plan_assignment.OrderBy
	ChunkSize int
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[plan_assignment.Name]*plan_assignment.PlanAssignment
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*plan_assignment.PlanAssignmentChange
}

func NewWatcher(client plan_assignment_client.PlanAssignmentServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Filter != nil {
			...
			cQuery.Filter = &plan_assignment.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "planAssignment-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[plan_assignment.Name]*plan_assignment.PlanAssignment{}
...
type WatcherEventChange struct {
	pre, post *plan_assignment.PlanAssignment
	name      *plan_assignment.Name
}

func NewAddWatcherEventChange(resource *plan_assignment.PlanAssignment) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *plan_assignment.PlanAssignment) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *plan_assignment.PlanAssignment) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *plan_assignment.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *plan_assignment.PlanAssignment {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *plan_assignment.PlanAssignment {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *plan_assignment.PlanAssignment {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *plan_assignment.PlanAssignment {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[plan_assignment.Name]*plan_assignment.PlanAssignment, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		planAssignment := currentChange.GetCurrent()
		byName[*planAssignment.GetName()] = planAssignment
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			planAssignment := change.GetCurrent()
			byName[*planAssignment.GetName()] = planAssignment
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, planAssignment := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(planAssignment))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[plan_assignment.Name]*plan_assignment.PlanAssignment{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *plan_assignment.PlanAssignment
		switch item := change.ChangeType.(type) {
		case *plan_assignment.PlanAssignmentChange_Added_:
			currentItem = item.Added.GetPlanAssignment()
		case *plan_assignment.PlanAssignmentChange_Current_:
			currentItem = item.Current.GetPlanAssignment()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*plan_assignment.PlanAssignmentChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *plan_assignment.PlanAssignmentChange_Added_:
			newItem := item.Added.GetPlanAssignment()
		...
		case *plan_assignment.PlanAssignmentChange_Modified_:
			updatedItem := item.Modified.GetPlanAssignment()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*plan_assignment.PlanAssignmentChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetPlanAssignmentName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(plan_assignment.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*plan_assignment.PlanAssignment_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*plan_assignment.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
