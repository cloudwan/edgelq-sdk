...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *accepted_plan.AcceptedPlan_FieldMask
	OrderBy   *accepted_plan.OrderBy
	ChunkSize int
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[accepted_plan.Name]*accepted_plan.AcceptedPlan
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*accepted_plan.AcceptedPlanChange
}

func NewWatcher(client accepted_plan_client.AcceptedPlanServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Filter != nil {
			...
			cQuery.Filter = &accepted_plan.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "acceptedPlan-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[accepted_plan.Name]*accepted_plan.AcceptedPlan{}
...
type WatcherEventChange struct {
	pre, post *accepted_plan.AcceptedPlan
	name      *accepted_plan.Name
}

func NewAddWatcherEventChange(resource *accepted_plan.AcceptedPlan) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *accepted_plan.AcceptedPlan) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *accepted_plan.AcceptedPlan) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *accepted_plan.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *accepted_plan.AcceptedPlan {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *accepted_plan.AcceptedPlan {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *accepted_plan.AcceptedPlan {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *accepted_plan.AcceptedPlan {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[accepted_plan.Name]*accepted_plan.AcceptedPlan, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		acceptedPlan := currentChange.GetCurrent()
		byName[*acceptedPlan.GetName()] = acceptedPlan
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			acceptedPlan := change.GetCurrent()
			byName[*acceptedPlan.GetName()] = acceptedPlan
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, acceptedPlan := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(acceptedPlan))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[accepted_plan.Name]*accepted_plan.AcceptedPlan{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *accepted_plan.AcceptedPlan
		switch item := change.ChangeType.(type) {
		case *accepted_plan.AcceptedPlanChange_Added_:
			currentItem = item.Added.GetAcceptedPlan()
		case *accepted_plan.AcceptedPlanChange_Current_:
			currentItem = item.Current.GetAcceptedPlan()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*accepted_plan.AcceptedPlanChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *accepted_plan.AcceptedPlanChange_Added_:
			newItem := item.Added.GetAcceptedPlan()
		...
		case *accepted_plan.AcceptedPlanChange_Modified_:
			updatedItem := item.Modified.GetAcceptedPlan()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*accepted_plan.AcceptedPlanChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetAcceptedPlanName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(accepted_plan.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*accepted_plan.AcceptedPlan_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*accepted_plan.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
