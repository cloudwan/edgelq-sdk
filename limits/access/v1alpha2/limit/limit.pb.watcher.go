...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *limit.Limit_FieldMask
	OrderBy   *limit.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *limit.ParentReference
	Filter *limit.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[limit.Name]*limit.Limit
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*limit.LimitChange
}

func NewWatcher(client limit_client.LimitServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &limit.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &limit.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "limit-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[limit.Name]*limit.Limit{}
...
type WatcherEventChange struct {
	pre, post *limit.Limit
	name      *limit.Name
}

func NewAddWatcherEventChange(resource *limit.Limit) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *limit.Limit) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *limit.Limit) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *limit.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *limit.Limit {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *limit.Limit {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *limit.Limit {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *limit.Limit {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[limit.Name]*limit.Limit, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		limit := currentChange.GetCurrent()
		byName[*limit.GetName()] = limit
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			limit := change.GetCurrent()
			byName[*limit.GetName()] = limit
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, limit := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(limit))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*limit.LimitChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *limit.LimitChange_Added_:
			newItem := item.Added.GetLimit()
		...
		case *limit.LimitChange_Modified_:
			updatedItem := item.Modified.GetLimit()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*limit.LimitChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetLimitName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(limit.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*limit.Limit_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*limit.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(limit_client.NewLimitServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(limit.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*limit.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*limit.ParentReference)
		}
		return params
	})
}
