...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *limit_pool.LimitPool_FieldMask
	OrderBy   *limit_pool.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *limit_pool.ParentReference
	Filter *limit_pool.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[limit_pool.Name]*limit_pool.LimitPool
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*limit_pool.LimitPoolChange
}
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &limit_pool.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &limit_pool.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "limitPool-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[limit_pool.Name]*limit_pool.LimitPool{}
...
type WatcherEventChange struct {
	pre, post *limit_pool.LimitPool
	name      *limit_pool.Name
}
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[limit_pool.Name]*limit_pool.LimitPool, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		limitPool := currentChange.GetCurrent()
		byName[*limitPool.GetName()] = limitPool
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			limitPool := change.GetCurrent()
			byName[*limitPool.GetName()] = limitPool
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, limitPool := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(limitPool))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[limit_pool.Name]*limit_pool.LimitPool{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *limit_pool.LimitPool
		switch item := change.ChangeType.(type) {
		case *limit_pool.LimitPoolChange_Added_:
			currentItem = item.Added.GetLimitPool()
		case *limit_pool.LimitPoolChange_Current_:
			currentItem = item.Current.GetLimitPool()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*limit_pool.LimitPoolChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *limit_pool.LimitPoolChange_Added_:
			newItem := item.Added.GetLimitPool()
		...
		case *limit_pool.LimitPoolChange_Modified_:
			updatedItem := item.Modified.GetLimitPool()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*limit_pool.LimitPoolChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetLimitPoolName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(limit_pool.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*limit_pool.LimitPool_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*limit_pool.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(limit_pool_client.NewLimitPoolServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(limit_pool.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*limit_pool.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*limit_pool.ParentReference)
		}
		return params
	})
}
