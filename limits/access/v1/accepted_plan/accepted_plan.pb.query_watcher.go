...
type QueryWatcherParams struct {
	...
	Cursor       *accepted_plan.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*accepted_plan.AcceptedPlanChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return accepted_plan.AcceptedPlanChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client accepted_plan_client.AcceptedPlanServiceClient,
...
func NewQueryWatcherWithIChan(id int, client accepted_plan_client.AcceptedPlanServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "acceptedPlan-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchAcceptedPlans(ctx, &accepted_plan_client.WatchAcceptedPlansRequest{
		...
		if err != nil {
			...
			pending := make([]*accepted_plan.AcceptedPlanChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetAcceptedPlanChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetAcceptedPlanChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(accepted_plan.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(accepted_plan.AcceptedPlanChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(accepted_plan.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*accepted_plan.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*accepted_plan.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*accepted_plan.Filter)
		}
		return NewQueryWatcherWithIChan(id, accepted_plan_client.NewAcceptedPlanServiceClient(cc), cfg, ch)
	})
}
