...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *retention_policy.RetentionPolicy_FieldMask
	OrderBy   *retention_policy.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *retention_policy.ParentName
	Filter *retention_policy.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[retention_policy.Name]*retention_policy.RetentionPolicy
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*retention_policy.RetentionPolicyChange
}

func NewWatcher(client retention_policy_client.RetentionPolicyServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &retention_policy.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &retention_policy.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "retentionPolicy-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[retention_policy.Name]*retention_policy.RetentionPolicy{}
...
type WatcherEventChange struct {
	pre, post *retention_policy.RetentionPolicy
	name      *retention_policy.Name
}

func NewAddWatcherEventChange(resource *retention_policy.RetentionPolicy) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *retention_policy.RetentionPolicy) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *retention_policy.RetentionPolicy) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *retention_policy.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *retention_policy.RetentionPolicy {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *retention_policy.RetentionPolicy {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *retention_policy.RetentionPolicy {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *retention_policy.RetentionPolicy {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[retention_policy.Name]*retention_policy.RetentionPolicy, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		retentionPolicy := currentChange.GetCurrent()
		byName[*retentionPolicy.GetName()] = retentionPolicy
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			retentionPolicy := change.GetCurrent()
			byName[*retentionPolicy.GetName()] = retentionPolicy
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, retentionPolicy := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(retentionPolicy))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[retention_policy.Name]*retention_policy.RetentionPolicy{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *retention_policy.RetentionPolicy
		switch item := change.ChangeType.(type) {
		case *retention_policy.RetentionPolicyChange_Added_:
			currentItem = item.Added.GetRetentionPolicy()
		case *retention_policy.RetentionPolicyChange_Current_:
			currentItem = item.Current.GetRetentionPolicy()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*retention_policy.RetentionPolicyChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *retention_policy.RetentionPolicyChange_Added_:
			newItem := item.Added.GetRetentionPolicy()
		...
		case *retention_policy.RetentionPolicyChange_Modified_:
			updatedItem := item.Modified.GetRetentionPolicy()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*retention_policy.RetentionPolicyChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetRetentionPolicyName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(retention_policy.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*retention_policy.RetentionPolicy_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*retention_policy.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(retention_policy_client.NewRetentionPolicyServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(retention_policy.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*retention_policy.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*retention_policy.ParentName)
		}
		return params
	})
}
