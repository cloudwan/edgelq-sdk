...
type QueryWatcherParams struct {
	...
	Cursor       *retention_policy.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*retention_policy.RetentionPolicyChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return retention_policy.RetentionPolicyChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client retention_policy_client.RetentionPolicyServiceClient,
...
func NewQueryWatcherWithIChan(id int, client retention_policy_client.RetentionPolicyServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "retentionPolicy-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchRetentionPolicies(ctx, &retention_policy_client.WatchRetentionPoliciesRequest{
		...
		if err != nil {
			...
			pending := make([]*retention_policy.RetentionPolicyChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetRetentionPolicyChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetRetentionPolicyChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(retention_policy.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(retention_policy.RetentionPolicyChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(retention_policy.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*retention_policy.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*retention_policy.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*retention_policy.Filter)
		}
		return NewQueryWatcherWithIChan(id, retention_policy_client.NewRetentionPolicyServiceClient(cc), cfg, ch)
	})
}
