...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*limit_pool.LimitPoolChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return limit_pool.LimitPoolChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client limit_pool_client.LimitPoolServiceClient,
...
func NewQueryWatcherWithIChan(id int, client limit_pool_client.LimitPoolServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "limitPool-query-watcher").
	...
	for {
		stream, err := qw.client.WatchLimitPools(ctx, &limit_pool_client.WatchLimitPoolsRequest{
		...
		if err != nil {
			...
			pending := make([]*limit_pool.LimitPoolChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetLimitPoolChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetLimitPoolChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(limit_pool.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(limit_pool.LimitPoolChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(limit_pool.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*limit_pool.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*limit_pool.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*limit_pool.Filter)
		}
		return NewQueryWatcherWithIChan(id, limit_pool_client.NewLimitPoolServiceClient(cc), cfg, ch)
	})
}
