...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return plan.PlanChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client plan_client.PlanServiceClient,
...
func NewQueryWatcherWithIChan(id int, client plan_client.PlanServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "plan-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchPlans(ctx, &plan_client.WatchPlansRequest{
		...
		if err != nil {
			...
			pending := make([]*plan.PlanChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetPlanChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(plan.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(plan.PlanChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(plan.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*plan.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*plan.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*plan.Filter)
		}
		return NewQueryWatcherWithIChan(id, plan_client.NewPlanServiceClient(cc), cfg, ch)
	})
}
