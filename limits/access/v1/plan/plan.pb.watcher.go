...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[plan.Name]*plan.Plan
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*plan.PlanChange
}

func NewWatcher(client plan_client.PlanServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "plan-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[plan.Name]*plan.Plan{}
...
func NewAddWatcherEventChange(resource *plan.Plan) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *plan.Plan) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *plan.Plan) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *plan.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *plan.Plan {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *plan.Plan {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *plan.Plan {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *plan.Plan {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[plan.Name]*plan.Plan, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		plan := currentChange.GetCurrent()
		byName[*plan.GetName()] = plan
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			plan := change.GetCurrent()
			byName[*plan.GetName()] = plan
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, plan := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(plan))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*plan.PlanChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *plan.PlanChange_Added_:
			newItem := item.Added.GetPlan()
		...
		case *plan.PlanChange_Modified_:
			updatedItem := item.Modified.GetPlan()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*plan.PlanChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetPlanName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(plan.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*plan.Plan_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*plan.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(plan_client.NewPlanServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(plan.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*plan.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*plan.ParentName)
		}
		return params
	})
}
