...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return limit.LimitChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client limit_client.LimitServiceClient,
...
func NewQueryWatcherWithIChan(id int, client limit_client.LimitServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "limit-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchLimits(ctx, &limit_client.WatchLimitsRequest{
		...
		if err != nil {
			...
			pending := make([]*limit.LimitChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetLimitChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(limit.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(limit.LimitChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(limit.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*limit.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*limit.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*limit.Filter)
		}
		return NewQueryWatcherWithIChan(id, limit_client.NewLimitServiceClient(cc), cfg, ch)
	})
}
