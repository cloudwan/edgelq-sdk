...
type QueryWatcherParams struct {
	...
	Cursor       *plan_assignment.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*plan_assignment.PlanAssignmentChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return plan_assignment.PlanAssignmentChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client plan_assignment_client.PlanAssignmentServiceClient,
...
func NewQueryWatcherWithIChan(id int, client plan_assignment_client.PlanAssignmentServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "planAssignment-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchPlanAssignments(ctx, &plan_assignment_client.WatchPlanAssignmentsRequest{
		...
		if err != nil {
			...
			pending := make([]*plan_assignment.PlanAssignmentChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetPlanAssignmentChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetPlanAssignmentChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(plan_assignment.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(plan_assignment.PlanAssignmentChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(plan_assignment.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*plan_assignment.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*plan_assignment.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*plan_assignment.Filter)
		}
		return NewQueryWatcherWithIChan(id, plan_assignment_client.NewPlanAssignmentServiceClient(cc), cfg, ch)
	})
}
