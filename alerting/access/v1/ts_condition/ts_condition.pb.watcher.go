...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *ts_condition.TsCondition_FieldMask
	OrderBy   *ts_condition.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *ts_condition.ParentName
	Filter *ts_condition.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[ts_condition.Name]*ts_condition.TsCondition
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*ts_condition.TsConditionChange
}

func NewWatcher(client ts_condition_client.TsConditionServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &ts_condition.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &ts_condition.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "tsCondition-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[ts_condition.Name]*ts_condition.TsCondition{}
...
type WatcherEventChange struct {
	pre, post *ts_condition.TsCondition
	name      *ts_condition.Name
}

func NewAddWatcherEventChange(resource *ts_condition.TsCondition) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *ts_condition.TsCondition) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *ts_condition.TsCondition) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *ts_condition.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *ts_condition.TsCondition {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *ts_condition.TsCondition {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *ts_condition.TsCondition {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *ts_condition.TsCondition {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[ts_condition.Name]*ts_condition.TsCondition, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		tsCondition := currentChange.GetCurrent()
		byName[*tsCondition.GetName()] = tsCondition
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			tsCondition := change.GetCurrent()
			byName[*tsCondition.GetName()] = tsCondition
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, tsCondition := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(tsCondition))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[ts_condition.Name]*ts_condition.TsCondition{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *ts_condition.TsCondition
		switch item := change.ChangeType.(type) {
		case *ts_condition.TsConditionChange_Added_:
			currentItem = item.Added.GetTsCondition()
		case *ts_condition.TsConditionChange_Current_:
			currentItem = item.Current.GetTsCondition()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*ts_condition.TsConditionChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *ts_condition.TsConditionChange_Added_:
			newItem := item.Added.GetTsCondition()
		...
		case *ts_condition.TsConditionChange_Modified_:
			updatedItem := item.Modified.GetTsCondition()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*ts_condition.TsConditionChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetTsConditionName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(ts_condition.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*ts_condition.TsCondition_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*ts_condition.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(ts_condition_client.NewTsConditionServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(ts_condition.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*ts_condition.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*ts_condition.ParentName)
		}
		return params
	})
}
