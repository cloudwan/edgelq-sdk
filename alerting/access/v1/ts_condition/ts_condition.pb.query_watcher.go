...
type QueryWatcherParams struct {
	...
	Cursor       *ts_condition.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*ts_condition.TsConditionChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return ts_condition.TsConditionChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client ts_condition_client.TsConditionServiceClient,
...
func NewQueryWatcherWithIChan(id int, client ts_condition_client.TsConditionServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "tsCondition-query-watcher").
	...
	for {
		stream, err := qw.client.WatchTsConditions(ctx, &ts_condition_client.WatchTsConditionsRequest{
		...
		if err != nil {
			...
			pending := make([]*ts_condition.TsConditionChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetTsConditionChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetTsConditionChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(ts_condition.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(ts_condition.TsConditionChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(ts_condition.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*ts_condition.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*ts_condition.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*ts_condition.Filter)
		}
		return NewQueryWatcherWithIChan(id, ts_condition_client.NewTsConditionServiceClient(cc), cfg, ch)
	})
}
