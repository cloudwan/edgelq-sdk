...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*ts_entry.TsEntryChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return ts_entry.TsEntryChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client ts_entry_client.TsEntryServiceClient,
...
func NewQueryWatcherWithIChan(id int, client ts_entry_client.TsEntryServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "tsEntry-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchTsEntries(ctx, &ts_entry_client.WatchTsEntriesRequest{
		...
		if err != nil {
			...
			pending := make([]*ts_entry.TsEntryChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetTsEntryChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetTsEntryChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(ts_entry.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(ts_entry.TsEntryChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(ts_entry.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*ts_entry.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*ts_entry.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*ts_entry.Filter)
		}
		return NewQueryWatcherWithIChan(id, ts_entry_client.NewTsEntryServiceClient(cc), cfg, ch)
	})
}
