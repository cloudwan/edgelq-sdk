...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *ts_entry.TsEntry_FieldMask
	OrderBy   *ts_entry.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *ts_entry.ParentName
	Filter *ts_entry.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[ts_entry.Name]*ts_entry.TsEntry
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*ts_entry.TsEntryChange
}

func NewWatcher(client ts_entry_client.TsEntryServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &ts_entry.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &ts_entry.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "tsEntry-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[ts_entry.Name]*ts_entry.TsEntry{}
...
type WatcherEventChange struct {
	pre, post *ts_entry.TsEntry
	name      *ts_entry.Name
}

func NewAddWatcherEventChange(resource *ts_entry.TsEntry) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *ts_entry.TsEntry) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *ts_entry.TsEntry) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *ts_entry.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *ts_entry.TsEntry {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *ts_entry.TsEntry {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *ts_entry.TsEntry {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *ts_entry.TsEntry {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[ts_entry.Name]*ts_entry.TsEntry, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		tsEntry := currentChange.GetCurrent()
		byName[*tsEntry.GetName()] = tsEntry
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			tsEntry := change.GetCurrent()
			byName[*tsEntry.GetName()] = tsEntry
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, tsEntry := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(tsEntry))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[ts_entry.Name]*ts_entry.TsEntry{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *ts_entry.TsEntry
		switch item := change.ChangeType.(type) {
		case *ts_entry.TsEntryChange_Added_:
			currentItem = item.Added.GetTsEntry()
		case *ts_entry.TsEntryChange_Current_:
			currentItem = item.Current.GetTsEntry()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*ts_entry.TsEntryChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *ts_entry.TsEntryChange_Added_:
			newItem := item.Added.GetTsEntry()
		...
		case *ts_entry.TsEntryChange_Modified_:
			updatedItem := item.Modified.GetTsEntry()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*ts_entry.TsEntryChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetTsEntryName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(ts_entry.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*ts_entry.TsEntry_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*ts_entry.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(ts_entry_client.NewTsEntryServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(ts_entry.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*ts_entry.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*ts_entry.ParentName)
		}
		return params
	})
}
