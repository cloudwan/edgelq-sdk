...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *log_condition.LogCondition_FieldMask
	OrderBy   *log_condition.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *log_condition.ParentName
	Filter *log_condition.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[log_condition.Name]*log_condition.LogCondition
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*log_condition.LogConditionChange
}

func NewWatcher(client log_condition_client.LogConditionServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &log_condition.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &log_condition.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "logCondition-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[log_condition.Name]*log_condition.LogCondition{}
...
type WatcherEventChange struct {
	pre, post *log_condition.LogCondition
	name      *log_condition.Name
}

func NewAddWatcherEventChange(resource *log_condition.LogCondition) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *log_condition.LogCondition) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *log_condition.LogCondition) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *log_condition.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *log_condition.LogCondition {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *log_condition.LogCondition {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *log_condition.LogCondition {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *log_condition.LogCondition {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[log_condition.Name]*log_condition.LogCondition, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		logCondition := currentChange.GetCurrent()
		byName[*logCondition.GetName()] = logCondition
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			logCondition := change.GetCurrent()
			byName[*logCondition.GetName()] = logCondition
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, logCondition := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(logCondition))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[log_condition.Name]*log_condition.LogCondition{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *log_condition.LogCondition
		switch item := change.ChangeType.(type) {
		case *log_condition.LogConditionChange_Added_:
			currentItem = item.Added.GetLogCondition()
		case *log_condition.LogConditionChange_Current_:
			currentItem = item.Current.GetLogCondition()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*log_condition.LogConditionChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *log_condition.LogConditionChange_Added_:
			newItem := item.Added.GetLogCondition()
		...
		case *log_condition.LogConditionChange_Modified_:
			updatedItem := item.Modified.GetLogCondition()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*log_condition.LogConditionChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetLogConditionName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(log_condition.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*log_condition.LogCondition_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*log_condition.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(log_condition_client.NewLogConditionServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(log_condition.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*log_condition.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*log_condition.ParentName)
		}
		return params
	})
}
