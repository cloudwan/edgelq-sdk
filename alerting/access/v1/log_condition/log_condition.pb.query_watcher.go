...
type QueryWatcherParams struct {
	...
	Cursor       *log_condition.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*log_condition.LogConditionChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return log_condition.LogConditionChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client log_condition_client.LogConditionServiceClient,
...
func NewQueryWatcherWithIChan(id int, client log_condition_client.LogConditionServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "logCondition-query-watcher").
	...
	for {
		stream, err := qw.client.WatchLogConditions(ctx, &log_condition_client.WatchLogConditionsRequest{
		...
		if err != nil {
			...
			pending := make([]*log_condition.LogConditionChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetLogConditionChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetLogConditionChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(log_condition.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(log_condition.LogConditionChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(log_condition.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*log_condition.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*log_condition.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*log_condition.Filter)
		}
		return NewQueryWatcherWithIChan(id, log_condition_client.NewLogConditionServiceClient(cc), cfg, ch)
	})
}
