...
type QueryWatcherParams struct {
	...
	Cursor       *ts_condition_template.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*ts_condition_template.TsConditionTemplateChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return ts_condition_template.TsConditionTemplateChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client ts_condition_template_client.TsConditionTemplateServiceClient,
...
func NewQueryWatcherWithIChan(id int, client ts_condition_template_client.TsConditionTemplateServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "tsConditionTemplate-query-watcher").
	...
	for {
		stream, err := qw.client.WatchTsConditionTemplates(ctx, &ts_condition_template_client.WatchTsConditionTemplatesRequest{
		...
		if err != nil {
			...
			pending := make([]*ts_condition_template.TsConditionTemplateChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetTsConditionTemplateChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetTsConditionTemplateChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(ts_condition_template.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(ts_condition_template.TsConditionTemplateChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(ts_condition_template.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*ts_condition_template.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*ts_condition_template.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*ts_condition_template.Filter)
		}
		return NewQueryWatcherWithIChan(id, ts_condition_template_client.NewTsConditionTemplateServiceClient(cc), cfg, ch)
	})
}
