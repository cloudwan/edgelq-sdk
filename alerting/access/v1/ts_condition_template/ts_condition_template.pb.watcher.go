...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *ts_condition_template.TsConditionTemplate_FieldMask
	OrderBy   *ts_condition_template.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *ts_condition_template.ParentName
	Filter *ts_condition_template.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[ts_condition_template.Name]*ts_condition_template.TsConditionTemplate
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*ts_condition_template.TsConditionTemplateChange
}

func NewWatcher(client ts_condition_template_client.TsConditionTemplateServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &ts_condition_template.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &ts_condition_template.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "tsConditionTemplate-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[ts_condition_template.Name]*ts_condition_template.TsConditionTemplate{}
...
type WatcherEventChange struct {
	pre, post *ts_condition_template.TsConditionTemplate
	name      *ts_condition_template.Name
}

func NewAddWatcherEventChange(resource *ts_condition_template.TsConditionTemplate) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *ts_condition_template.TsConditionTemplate) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *ts_condition_template.TsConditionTemplate) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *ts_condition_template.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *ts_condition_template.TsConditionTemplate {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *ts_condition_template.TsConditionTemplate {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *ts_condition_template.TsConditionTemplate {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *ts_condition_template.TsConditionTemplate {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[ts_condition_template.Name]*ts_condition_template.TsConditionTemplate, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		tsConditionTemplate := currentChange.GetCurrent()
		byName[*tsConditionTemplate.GetName()] = tsConditionTemplate
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			tsConditionTemplate := change.GetCurrent()
			byName[*tsConditionTemplate.GetName()] = tsConditionTemplate
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, tsConditionTemplate := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(tsConditionTemplate))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[ts_condition_template.Name]*ts_condition_template.TsConditionTemplate{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *ts_condition_template.TsConditionTemplate
		switch item := change.ChangeType.(type) {
		case *ts_condition_template.TsConditionTemplateChange_Added_:
			currentItem = item.Added.GetTsConditionTemplate()
		case *ts_condition_template.TsConditionTemplateChange_Current_:
			currentItem = item.Current.GetTsConditionTemplate()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*ts_condition_template.TsConditionTemplateChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *ts_condition_template.TsConditionTemplateChange_Added_:
			newItem := item.Added.GetTsConditionTemplate()
		...
		case *ts_condition_template.TsConditionTemplateChange_Modified_:
			updatedItem := item.Modified.GetTsConditionTemplate()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*ts_condition_template.TsConditionTemplateChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetTsConditionTemplateName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(ts_condition_template.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*ts_condition_template.TsConditionTemplate_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*ts_condition_template.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(ts_condition_template_client.NewTsConditionTemplateServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(ts_condition_template.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*ts_condition_template.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*ts_condition_template.ParentName)
		}
		return params
	})
}
