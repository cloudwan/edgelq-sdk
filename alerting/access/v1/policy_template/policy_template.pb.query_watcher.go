...
type QueryWatcherParams struct {
	...
	Cursor       *policy_template.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*policy_template.PolicyTemplateChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return policy_template.PolicyTemplateChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client policy_template_client.PolicyTemplateServiceClient,
...
func NewQueryWatcherWithIChan(id int, client policy_template_client.PolicyTemplateServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "policyTemplate-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchPolicyTemplates(ctx, &policy_template_client.WatchPolicyTemplatesRequest{
		...
		if err != nil {
			...
			pending := make([]*policy_template.PolicyTemplateChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetPolicyTemplateChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetPolicyTemplateChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(policy_template.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(policy_template.PolicyTemplateChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(policy_template.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*policy_template.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*policy_template.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*policy_template.Filter)
		}
		return NewQueryWatcherWithIChan(id, policy_template_client.NewPolicyTemplateServiceClient(cc), cfg, ch)
	})
}
