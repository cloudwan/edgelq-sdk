...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *policy_template.PolicyTemplate_FieldMask
	OrderBy   *policy_template.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *policy_template.ParentName
	Filter *policy_template.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[policy_template.Name]*policy_template.PolicyTemplate
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*policy_template.PolicyTemplateChange
}

func NewWatcher(client policy_template_client.PolicyTemplateServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &policy_template.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &policy_template.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "policyTemplate-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[policy_template.Name]*policy_template.PolicyTemplate{}
...
type WatcherEventChange struct {
	pre, post *policy_template.PolicyTemplate
	name      *policy_template.Name
}

func NewAddWatcherEventChange(resource *policy_template.PolicyTemplate) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *policy_template.PolicyTemplate) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *policy_template.PolicyTemplate) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *policy_template.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *policy_template.PolicyTemplate {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *policy_template.PolicyTemplate {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *policy_template.PolicyTemplate {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *policy_template.PolicyTemplate {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[policy_template.Name]*policy_template.PolicyTemplate, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		policyTemplate := currentChange.GetCurrent()
		byName[*policyTemplate.GetName()] = policyTemplate
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			policyTemplate := change.GetCurrent()
			byName[*policyTemplate.GetName()] = policyTemplate
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, policyTemplate := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(policyTemplate))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[policy_template.Name]*policy_template.PolicyTemplate{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *policy_template.PolicyTemplate
		switch item := change.ChangeType.(type) {
		case *policy_template.PolicyTemplateChange_Added_:
			currentItem = item.Added.GetPolicyTemplate()
		case *policy_template.PolicyTemplateChange_Current_:
			currentItem = item.Current.GetPolicyTemplate()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*policy_template.PolicyTemplateChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *policy_template.PolicyTemplateChange_Added_:
			newItem := item.Added.GetPolicyTemplate()
		...
		case *policy_template.PolicyTemplateChange_Modified_:
			updatedItem := item.Modified.GetPolicyTemplate()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*policy_template.PolicyTemplateChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetPolicyTemplateName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(policy_template.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*policy_template.PolicyTemplate_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*policy_template.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(policy_template_client.NewPolicyTemplateServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(policy_template.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*policy_template.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*policy_template.ParentName)
		}
		return params
	})
}
