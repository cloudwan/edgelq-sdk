...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[log_condition_template.Name]*log_condition_template.LogConditionTemplate
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*log_condition_template.LogConditionTemplateChange
}

func NewWatcher(client log_condition_template_client.LogConditionTemplateServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &log_condition_template.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &log_condition_template.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "logConditionTemplate-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[log_condition_template.Name]*log_condition_template.LogConditionTemplate{}
...
type WatcherEventChange struct {
	pre, post *log_condition_template.LogConditionTemplate
	name      *log_condition_template.Name
}

func NewAddWatcherEventChange(resource *log_condition_template.LogConditionTemplate) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *log_condition_template.LogConditionTemplate) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *log_condition_template.LogConditionTemplate) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *log_condition_template.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *log_condition_template.LogConditionTemplate {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *log_condition_template.LogConditionTemplate {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *log_condition_template.LogConditionTemplate {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *log_condition_template.LogConditionTemplate {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[log_condition_template.Name]*log_condition_template.LogConditionTemplate, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		logConditionTemplate := currentChange.GetCurrent()
		byName[*logConditionTemplate.GetName()] = logConditionTemplate
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			logConditionTemplate := change.GetCurrent()
			byName[*logConditionTemplate.GetName()] = logConditionTemplate
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, logConditionTemplate := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(logConditionTemplate))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[log_condition_template.Name]*log_condition_template.LogConditionTemplate{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *log_condition_template.LogConditionTemplate
		switch item := change.ChangeType.(type) {
		case *log_condition_template.LogConditionTemplateChange_Added_:
			currentItem = item.Added.GetLogConditionTemplate()
		case *log_condition_template.LogConditionTemplateChange_Current_:
			currentItem = item.Current.GetLogConditionTemplate()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*log_condition_template.LogConditionTemplateChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *log_condition_template.LogConditionTemplateChange_Added_:
			newItem := item.Added.GetLogConditionTemplate()
		...
		case *log_condition_template.LogConditionTemplateChange_Modified_:
			updatedItem := item.Modified.GetLogConditionTemplate()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*log_condition_template.LogConditionTemplateChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetLogConditionTemplateName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(log_condition_template.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		return NewWatcher(log_condition_template_client.NewLogConditionTemplateServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(log_condition_template.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*log_condition_template.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*log_condition_template.ParentName)
		}
		return params
	})
}
