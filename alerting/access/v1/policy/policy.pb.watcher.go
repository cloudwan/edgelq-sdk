...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *policy.Policy_FieldMask
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[policy.Name]*policy.Policy
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*policy.PolicyChange
}

func NewWatcher(client policy_client.PolicyServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "policy-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[policy.Name]*policy.Policy{}
...
type WatcherEventChange struct {
	pre, post *policy.Policy
	name      *policy.Name
}

func NewAddWatcherEventChange(resource *policy.Policy) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *policy.Policy) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *policy.Policy) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *policy.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *policy.Policy {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *policy.Policy {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *policy.Policy {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *policy.Policy {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[policy.Name]*policy.Policy, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		policy := currentChange.GetCurrent()
		byName[*policy.GetName()] = policy
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			policy := change.GetCurrent()
			byName[*policy.GetName()] = policy
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, policy := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(policy))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*policy.PolicyChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *policy.PolicyChange_Added_:
			newItem := item.Added.GetPolicy()
		...
		case *policy.PolicyChange_Modified_:
			updatedItem := item.Modified.GetPolicy()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*policy.PolicyChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetPolicyName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(policy.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*policy.Policy_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*policy.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(policy_client.NewPolicyServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(policy.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*policy.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*policy.ParentName)
		}
		return params
	})
}
