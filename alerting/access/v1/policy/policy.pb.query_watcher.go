...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return policy.PolicyChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client policy_client.PolicyServiceClient,
...
func NewQueryWatcherWithIChan(id int, client policy_client.PolicyServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "policy-query-watcher").
	...
	for {
		stream, err := qw.client.WatchPolicies(ctx, &policy_client.WatchPoliciesRequest{
		...
		if err != nil {
			...
			pending := make([]*policy.PolicyChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetPolicyChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(policy.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(policy.PolicyChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(policy.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*policy.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*policy.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*policy.Filter)
		}
		return NewQueryWatcherWithIChan(id, policy_client.NewPolicyServiceClient(cc), cfg, ch)
	})
}
