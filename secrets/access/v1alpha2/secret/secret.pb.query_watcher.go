...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return secret.SecretChangeList(e.Changes)
}
...
func NewQueryWatcherWithIChan(id int, client secret_client.SecretServiceClient,
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(secret.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(secret.SecretChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(secret.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*secret.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*secret.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*secret.Filter)
		}
		return NewQueryWatcherWithIChan(id, secret_client.NewSecretServiceClient(cc), cfg, ch)
	})
}
