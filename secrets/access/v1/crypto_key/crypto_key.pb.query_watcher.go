...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*crypto_key.CryptoKeyChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return crypto_key.CryptoKeyChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client crypto_key_client.CryptoKeyServiceClient,
...
func NewQueryWatcherWithIChan(id int, client crypto_key_client.CryptoKeyServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "cryptoKey-query-watcher").
	...
	for {
		stream, err := qw.client.WatchCryptoKeys(ctx, &crypto_key_client.WatchCryptoKeysRequest{
		...
		if err != nil {
			...
			pending := make([]*crypto_key.CryptoKeyChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetCryptoKeyChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetCryptoKeyChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(crypto_key.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(crypto_key.CryptoKeyChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(crypto_key.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*crypto_key.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*crypto_key.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*crypto_key.Filter)
		}
		return NewQueryWatcherWithIChan(id, crypto_key_client.NewCryptoKeyServiceClient(cc), cfg, ch)
	})
}
