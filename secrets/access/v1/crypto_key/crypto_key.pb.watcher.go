...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *crypto_key.CryptoKey_FieldMask
	OrderBy   *crypto_key.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *crypto_key.ParentName
	Filter *crypto_key.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[crypto_key.Name]*crypto_key.CryptoKey
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*crypto_key.CryptoKeyChange
}

func NewWatcher(client crypto_key_client.CryptoKeyServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &crypto_key.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &crypto_key.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "cryptoKey-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[crypto_key.Name]*crypto_key.CryptoKey{}
...
type WatcherEventChange struct {
	pre, post *crypto_key.CryptoKey
	name      *crypto_key.Name
}

func NewAddWatcherEventChange(resource *crypto_key.CryptoKey) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *crypto_key.CryptoKey) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *crypto_key.CryptoKey) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *crypto_key.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *crypto_key.CryptoKey {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *crypto_key.CryptoKey {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *crypto_key.CryptoKey {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *crypto_key.CryptoKey {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[crypto_key.Name]*crypto_key.CryptoKey, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		cryptoKey := currentChange.GetCurrent()
		byName[*cryptoKey.GetName()] = cryptoKey
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			cryptoKey := change.GetCurrent()
			byName[*cryptoKey.GetName()] = cryptoKey
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, cryptoKey := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(cryptoKey))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[crypto_key.Name]*crypto_key.CryptoKey{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *crypto_key.CryptoKey
		switch item := change.ChangeType.(type) {
		case *crypto_key.CryptoKeyChange_Added_:
			currentItem = item.Added.GetCryptoKey()
		case *crypto_key.CryptoKeyChange_Current_:
			currentItem = item.Current.GetCryptoKey()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*crypto_key.CryptoKeyChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *crypto_key.CryptoKeyChange_Added_:
			newItem := item.Added.GetCryptoKey()
		...
		case *crypto_key.CryptoKeyChange_Modified_:
			updatedItem := item.Modified.GetCryptoKey()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*crypto_key.CryptoKeyChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetCryptoKeyName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(crypto_key.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*crypto_key.CryptoKey_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*crypto_key.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(crypto_key_client.NewCryptoKeyServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(crypto_key.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*crypto_key.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*crypto_key.ParentName)
		}
		return params
	})
}
