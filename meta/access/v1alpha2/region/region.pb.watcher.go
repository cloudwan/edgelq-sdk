...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *region.Region_FieldMask
	OrderBy   *region.OrderBy
	ChunkSize int
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[region.Name]*region.Region
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*region.RegionChange
}

func NewWatcher(client region_client.RegionServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "region-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[region.Name]*region.Region{}
...
type WatcherEventChange struct {
	pre, post *region.Region
	name      *region.Name
}

func NewAddWatcherEventChange(resource *region.Region) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *region.Region) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *region.Region) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *region.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *region.Region {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *region.Region {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *region.Region {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *region.Region {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[region.Name]*region.Region, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		region := currentChange.GetCurrent()
		byName[*region.GetName()] = region
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			region := change.GetCurrent()
			byName[*region.GetName()] = region
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, region := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(region))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*region.RegionChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *region.RegionChange_Added_:
			newItem := item.Added.GetRegion()
		...
		case *region.RegionChange_Modified_:
			updatedItem := item.Modified.GetRegion()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*region.RegionChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetRegionName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(region.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*region.Region_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*region.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(region_client.NewRegionServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(region.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*region.Filter)
		}
		return params
	})
}
