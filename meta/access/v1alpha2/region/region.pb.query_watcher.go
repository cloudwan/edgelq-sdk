...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return region.RegionChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client region_client.RegionServiceClient,
...
func NewQueryWatcherWithIChan(id int, client region_client.RegionServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "region-query-watcher").
	...
	for {
		stream, err := qw.client.WatchRegions(ctx, &region_client.WatchRegionsRequest{
		...
		if err != nil {
			...
			pending := make([]*region.RegionChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetRegionChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(region.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(region.RegionChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(region.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*region.PagerCursor)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*region.Filter)
		}
		return NewQueryWatcherWithIChan(id, region_client.NewRegionServiceClient(cc), cfg, ch)
	})
}
