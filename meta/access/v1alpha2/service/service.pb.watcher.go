...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *service.Service_FieldMask
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[service.Name]*service.Service
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*service.ServiceChange
}

func NewWatcher(client service_client.ServiceServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "service-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[service.Name]*service.Service{}
...
type WatcherEventChange struct {
	pre, post *service.Service
	name      *service.Name
}

func NewAddWatcherEventChange(resource *service.Service) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *service.Service) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *service.Service) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *service.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *service.Service {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *service.Service {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *service.Service {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *service.Service {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[service.Name]*service.Service, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		service := currentChange.GetCurrent()
		byName[*service.GetName()] = service
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			service := change.GetCurrent()
			byName[*service.GetName()] = service
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, service := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(service))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[service.Name]*service.Service{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *service.Service
		switch item := change.ChangeType.(type) {
		case *service.ServiceChange_Added_:
			currentItem = item.Added.GetService()
		case *service.ServiceChange_Current_:
			currentItem = item.Current.GetService()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*service.ServiceChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *service.ServiceChange_Added_:
			newItem := item.Added.GetService()
		...
		case *service.ServiceChange_Modified_:
			updatedItem := item.Modified.GetService()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*service.ServiceChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetServiceName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(service.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*service.Service_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*service.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(service_client.NewServiceServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(service.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*service.Filter)
		}
		return params
	})
}
