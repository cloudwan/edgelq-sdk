...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return service.ServiceChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client service_client.ServiceServiceClient,
...
func NewQueryWatcherWithIChan(id int, client service_client.ServiceServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "service-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchServices(ctx, &service_client.WatchServicesRequest{
		...
		if err != nil {
			...
			pending := make([]*service.ServiceChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetServiceChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetServiceChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(service.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(service.ServiceChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(service.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*service.PagerCursor)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*service.Filter)
		}
		return NewQueryWatcherWithIChan(id, service_client.NewServiceServiceClient(cc), cfg, ch)
	})
}
