...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*deployment.DeploymentChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return deployment.DeploymentChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client deployment_client.DeploymentServiceClient,
...
func NewQueryWatcherWithIChan(id int, client deployment_client.DeploymentServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "deployment-query-watcher").
	...
	for {
		stream, err := qw.client.WatchDeployments(ctx, &deployment_client.WatchDeploymentsRequest{
		...
		if err != nil {
			...
			pending := make([]*deployment.DeploymentChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetDeploymentChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetDeploymentChanges(),
...
func init() {
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(deployment.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		return NewQueryWatcherWithIChan(id, deployment_client.NewDeploymentServiceClient(cc), cfg, ch)
	})
}
