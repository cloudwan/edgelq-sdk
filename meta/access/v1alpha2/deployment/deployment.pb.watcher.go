...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[deployment.Name]*deployment.Deployment
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*deployment.DeploymentChange
}

func NewWatcher(client deployment_client.DeploymentServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &deployment.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &deployment.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "deployment-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[deployment.Name]*deployment.Deployment{}
...
type WatcherEventChange struct {
	pre, post *deployment.Deployment
	name      *deployment.Name
}

func NewAddWatcherEventChange(resource *deployment.Deployment) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *deployment.Deployment) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *deployment.Deployment) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *deployment.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *deployment.Deployment {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *deployment.Deployment {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *deployment.Deployment {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *deployment.Deployment {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[deployment.Name]*deployment.Deployment, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		deployment := currentChange.GetCurrent()
		byName[*deployment.GetName()] = deployment
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			deployment := change.GetCurrent()
			byName[*deployment.GetName()] = deployment
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, deployment := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(deployment))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[deployment.Name]*deployment.Deployment{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *deployment.Deployment
		switch item := change.ChangeType.(type) {
		case *deployment.DeploymentChange_Added_:
			currentItem = item.Added.GetDeployment()
		case *deployment.DeploymentChange_Current_:
			currentItem = item.Current.GetDeployment()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*deployment.DeploymentChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *deployment.DeploymentChange_Added_:
			newItem := item.Added.GetDeployment()
		...
		case *deployment.DeploymentChange_Modified_:
			updatedItem := item.Modified.GetDeployment()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(deployment.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*deployment.Deployment_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*deployment.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(deployment_client.NewDeploymentServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(deployment.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*deployment.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*deployment.ParentReference)
		}
		return params
	})
}
