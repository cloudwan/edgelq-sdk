...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *resource.Resource_FieldMask
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[resource.Name]*resource.Resource
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*resource.ResourceChange
}

func NewWatcher(client resource_client.ResourceServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &resource.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &resource.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "resource-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[resource.Name]*resource.Resource{}
...
type WatcherEventChange struct {
	pre, post *resource.Resource
	name      *resource.Name
}

func NewAddWatcherEventChange(resource *resource.Resource) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *resource.Resource) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *resource.Resource) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *resource.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *resource.Resource {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *resource.Resource {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *resource.Resource {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *resource.Resource {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[resource.Name]*resource.Resource, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		resource := currentChange.GetCurrent()
	...
	for _, change := range src.Changes {
		if change.IsDelete() {
			...
			resource := change.GetCurrent()
			byName[*resource.GetName()] = resource
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, resource := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(resource))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[resource.Name]*resource.Resource{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *resource.Resource
		switch item := change.ChangeType.(type) {
		case *resource.ResourceChange_Added_:
			currentItem = item.Added.GetResource()
		case *resource.ResourceChange_Current_:
			currentItem = item.Current.GetResource()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*resource.ResourceChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *resource.ResourceChange_Added_:
			newItem := item.Added.GetResource()
		...
		case *resource.ResourceChange_Modified_:
			updatedItem := item.Modified.GetResource()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*resource.ResourceChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetResourceName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(resource.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*resource.Resource_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*resource.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(resource_client.NewResourceServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(resource.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*resource.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*resource.ParentReference)
		}
		return params
	})
}
