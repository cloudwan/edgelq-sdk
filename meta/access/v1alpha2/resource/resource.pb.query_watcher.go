...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return resource.ResourceChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client resource_client.ResourceServiceClient,
...
func NewQueryWatcherWithIChan(id int, client resource_client.ResourceServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "resource-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchResources(ctx, &resource_client.WatchResourcesRequest{
		...
		if err != nil {
			...
			pending := make([]*resource.ResourceChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetResourceChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetResourceChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(resource.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(resource.ResourceChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(resource.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*resource.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*resource.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*resource.Filter)
		}
		return NewQueryWatcherWithIChan(id, resource_client.NewResourceServiceClient(cc), cfg, ch)
	})
}
