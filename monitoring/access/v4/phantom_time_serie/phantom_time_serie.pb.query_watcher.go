...
type QueryWatcherParams struct {
	...
	Cursor       *phantom_time_serie.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*phantom_time_serie.PhantomTimeSerieChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return phantom_time_serie.PhantomTimeSerieChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client phantom_time_serie_client.PhantomTimeSerieServiceClient,
...
func NewQueryWatcherWithIChan(id int, client phantom_time_serie_client.PhantomTimeSerieServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "phantomTimeSerie-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchPhantomTimeSeries(ctx, &phantom_time_serie_client.WatchPhantomTimeSeriesRequest{
		...
		if err != nil {
			...
			pending := make([]*phantom_time_serie.PhantomTimeSerieChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetPhantomTimeSerieChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetPhantomTimeSerieChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(phantom_time_serie.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(phantom_time_serie.PhantomTimeSerieChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(phantom_time_serie.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*phantom_time_serie.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*phantom_time_serie.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*phantom_time_serie.Filter)
		}
		return NewQueryWatcherWithIChan(id, phantom_time_serie_client.NewPhantomTimeSerieServiceClient(cc), cfg, ch)
	})
}
