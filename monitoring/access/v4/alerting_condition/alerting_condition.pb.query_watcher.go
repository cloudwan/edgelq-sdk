...
type QueryWatcherParams struct {
	...
	Cursor       *alerting_condition.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*alerting_condition.AlertingConditionChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return alerting_condition.AlertingConditionChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client alerting_condition_client.AlertingConditionServiceClient,
...
func NewQueryWatcherWithIChan(id int, client alerting_condition_client.AlertingConditionServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "alertingCondition-query-watcher").
	...
	for {
		stream, err := qw.client.WatchAlertingConditions(ctx, &alerting_condition_client.WatchAlertingConditionsRequest{
		...
		if err != nil {
			...
			pending := make([]*alerting_condition.AlertingConditionChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetAlertingConditionChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetAlertingConditionChanges(),
...
func init() {
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(alerting_condition.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		return NewQueryWatcherWithIChan(id, alerting_condition_client.NewAlertingConditionServiceClient(cc), cfg, ch)
	})
}
