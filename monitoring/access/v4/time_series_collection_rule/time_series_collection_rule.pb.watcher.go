...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *time_series_collection_rule.TimeSeriesCollectionRule_FieldMask
	OrderBy   *time_series_collection_rule.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *time_series_collection_rule.ParentName
	Filter *time_series_collection_rule.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[time_series_collection_rule.Name]*time_series_collection_rule.TimeSeriesCollectionRule
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*time_series_collection_rule.TimeSeriesCollectionRuleChange
}
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &time_series_collection_rule.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &time_series_collection_rule.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "timeSeriesCollectionRule-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[time_series_collection_rule.Name]*time_series_collection_rule.TimeSeriesCollectionRule{}
...
type WatcherEventChange struct {
	pre, post *time_series_collection_rule.TimeSeriesCollectionRule
	name      *time_series_collection_rule.Name
}
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[time_series_collection_rule.Name]*time_series_collection_rule.TimeSeriesCollectionRule, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		timeSeriesCollectionRule := currentChange.GetCurrent()
		byName[*timeSeriesCollectionRule.GetName()] = timeSeriesCollectionRule
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			timeSeriesCollectionRule := change.GetCurrent()
			byName[*timeSeriesCollectionRule.GetName()] = timeSeriesCollectionRule
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, timeSeriesCollectionRule := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(timeSeriesCollectionRule))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[time_series_collection_rule.Name]*time_series_collection_rule.TimeSeriesCollectionRule{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *time_series_collection_rule.TimeSeriesCollectionRule
		switch item := change.ChangeType.(type) {
		case *time_series_collection_rule.TimeSeriesCollectionRuleChange_Added_:
			currentItem = item.Added.GetTimeSeriesCollectionRule()
		case *time_series_collection_rule.TimeSeriesCollectionRuleChange_Current_:
			currentItem = item.Current.GetTimeSeriesCollectionRule()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*time_series_collection_rule.TimeSeriesCollectionRuleChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *time_series_collection_rule.TimeSeriesCollectionRuleChange_Added_:
			newItem := item.Added.GetTimeSeriesCollectionRule()
		...
		case *time_series_collection_rule.TimeSeriesCollectionRuleChange_Modified_:
			updatedItem := item.Modified.GetTimeSeriesCollectionRule()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*time_series_collection_rule.TimeSeriesCollectionRuleChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetTimeSeriesCollectionRuleName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(time_series_collection_rule.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*time_series_collection_rule.TimeSeriesCollectionRule_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*time_series_collection_rule.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(time_series_collection_rule_client.NewTimeSeriesCollectionRuleServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(time_series_collection_rule.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*time_series_collection_rule.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*time_series_collection_rule.ParentName)
		}
		return params
	})
}
