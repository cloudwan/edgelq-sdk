...
type QueryWatcherParams struct {
	...
	Cursor       *time_series_collection_rule.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*time_series_collection_rule.TimeSeriesCollectionRuleChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return time_series_collection_rule.TimeSeriesCollectionRuleChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client time_series_collection_rule_client.TimeSeriesCollectionRuleServiceClient,
...
func NewQueryWatcherWithIChan(id int, client time_series_collection_rule_client.TimeSeriesCollectionRuleServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "timeSeriesCollectionRule-query-watcher").
	...
	for {
		stream, err := qw.client.WatchTimeSeriesCollectionRules(ctx, &time_series_collection_rule_client.WatchTimeSeriesCollectionRulesRequest{
		...
		if err != nil {
			...
			pending := make([]*time_series_collection_rule.TimeSeriesCollectionRuleChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetTimeSeriesCollectionRuleChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetTimeSeriesCollectionRuleChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(time_series_collection_rule.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(time_series_collection_rule.TimeSeriesCollectionRuleChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(time_series_collection_rule.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*time_series_collection_rule.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*time_series_collection_rule.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*time_series_collection_rule.Filter)
		}
		return NewQueryWatcherWithIChan(id, time_series_collection_rule_client.NewTimeSeriesCollectionRuleServiceClient(cc), cfg, ch)
	})
}
