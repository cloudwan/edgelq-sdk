...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return bucket.BucketChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client bucket_client.BucketServiceClient,
...
func NewQueryWatcherWithIChan(id int, client bucket_client.BucketServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "bucket-query-watcher").
	...
	for {
		stream, err := qw.client.WatchBuckets(ctx, &bucket_client.WatchBucketsRequest{
		...
		if err != nil {
			...
			pending := make([]*bucket.BucketChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetBucketChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(bucket.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(bucket.BucketChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(bucket.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*bucket.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*bucket.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*bucket.Filter)
		}
		return NewQueryWatcherWithIChan(id, bucket_client.NewBucketServiceClient(cc), cfg, ch)
	})
}
