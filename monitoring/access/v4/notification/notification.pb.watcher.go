...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &notification.ParentName{}
...
func init() {
	...
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(notification.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*notification.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*notification.ParentName)
		}
		return params
	})
}
