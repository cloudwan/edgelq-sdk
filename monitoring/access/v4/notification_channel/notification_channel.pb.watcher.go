...
type WatcherFilterParams struct {
	Parent *notification_channel.ParentName
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &notification_channel.ParentName{}
...
func init() {
	...
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(notification_channel.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*notification_channel.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*notification_channel.ParentName)
		}
		return params
	})
}
