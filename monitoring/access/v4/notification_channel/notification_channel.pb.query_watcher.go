...
type QueryWatcherParams struct {
	...
	Cursor       *notification_channel.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*notification_channel.NotificationChannelChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return notification_channel.NotificationChannelChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client notification_channel_client.NotificationChannelServiceClient,
...
func NewQueryWatcherWithIChan(id int, client notification_channel_client.NotificationChannelServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "notificationChannel-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchNotificationChannels(ctx, &notification_channel_client.WatchNotificationChannelsRequest{
		...
		if err != nil {
			...
			pending := make([]*notification_channel.NotificationChannelChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetNotificationChannelChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetNotificationChannelChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(notification_channel.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(notification_channel.NotificationChannelChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(notification_channel.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*notification_channel.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*notification_channel.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*notification_channel.Filter)
		}
		return NewQueryWatcherWithIChan(id, notification_channel_client.NewNotificationChannelServiceClient(cc), cfg, ch)
	})
}
