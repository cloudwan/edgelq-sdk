...
type QueryWatcherParams struct {
	...
	Cursor       *monitored_resource_descriptor.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*monitored_resource_descriptor.MonitoredResourceDescriptorChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return monitored_resource_descriptor.MonitoredResourceDescriptorChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client monitored_resource_descriptor_client.MonitoredResourceDescriptorServiceClient,
...
func NewQueryWatcherWithIChan(id int, client monitored_resource_descriptor_client.MonitoredResourceDescriptorServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "monitoredResourceDescriptor-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchMonitoredResourceDescriptors(ctx, &monitored_resource_descriptor_client.WatchMonitoredResourceDescriptorsRequest{
		...
		if err != nil {
			...
			pending := make([]*monitored_resource_descriptor.MonitoredResourceDescriptorChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetMonitoredResourceDescriptorChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetMonitoredResourceDescriptorChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(monitored_resource_descriptor.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(monitored_resource_descriptor.MonitoredResourceDescriptorChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(monitored_resource_descriptor.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*monitored_resource_descriptor.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*monitored_resource_descriptor.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*monitored_resource_descriptor.Filter)
		}
		return NewQueryWatcherWithIChan(id, monitored_resource_descriptor_client.NewMonitoredResourceDescriptorServiceClient(cc), cfg, ch)
	})
}
