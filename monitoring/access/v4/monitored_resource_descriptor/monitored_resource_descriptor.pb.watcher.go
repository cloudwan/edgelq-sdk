...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *monitored_resource_descriptor.MonitoredResourceDescriptor_FieldMask
	OrderBy   *monitored_resource_descriptor.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *monitored_resource_descriptor.ParentName
	Filter *monitored_resource_descriptor.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[monitored_resource_descriptor.Name]*monitored_resource_descriptor.MonitoredResourceDescriptor
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*monitored_resource_descriptor.MonitoredResourceDescriptorChange
}
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &monitored_resource_descriptor.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &monitored_resource_descriptor.Filter{}
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[monitored_resource_descriptor.Name]*monitored_resource_descriptor.MonitoredResourceDescriptor{}
...
type WatcherEventChange struct {
	pre, post *monitored_resource_descriptor.MonitoredResourceDescriptor
	name      *monitored_resource_descriptor.Name
}

func NewAddWatcherEventChange(resource *monitored_resource_descriptor.MonitoredResourceDescriptor) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *monitored_resource_descriptor.MonitoredResourceDescriptor) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *monitored_resource_descriptor.MonitoredResourceDescriptor) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *monitored_resource_descriptor.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *monitored_resource_descriptor.MonitoredResourceDescriptor {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *monitored_resource_descriptor.MonitoredResourceDescriptor {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *monitored_resource_descriptor.MonitoredResourceDescriptor {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *monitored_resource_descriptor.MonitoredResourceDescriptor {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[monitored_resource_descriptor.Name]*monitored_resource_descriptor.MonitoredResourceDescriptor, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		monitoredResourceDescriptor := currentChange.GetCurrent()
		byName[*monitoredResourceDescriptor.GetName()] = monitoredResourceDescriptor
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			monitoredResourceDescriptor := change.GetCurrent()
			byName[*monitoredResourceDescriptor.GetName()] = monitoredResourceDescriptor
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, monitoredResourceDescriptor := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(monitoredResourceDescriptor))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[monitored_resource_descriptor.Name]*monitored_resource_descriptor.MonitoredResourceDescriptor{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *monitored_resource_descriptor.MonitoredResourceDescriptor
		switch item := change.ChangeType.(type) {
		case *monitored_resource_descriptor.MonitoredResourceDescriptorChange_Added_:
			currentItem = item.Added.GetMonitoredResourceDescriptor()
		case *monitored_resource_descriptor.MonitoredResourceDescriptorChange_Current_:
			currentItem = item.Current.GetMonitoredResourceDescriptor()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*monitored_resource_descriptor.MonitoredResourceDescriptorChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *monitored_resource_descriptor.MonitoredResourceDescriptorChange_Added_:
			newItem := item.Added.GetMonitoredResourceDescriptor()
		...
		case *monitored_resource_descriptor.MonitoredResourceDescriptorChange_Modified_:
			updatedItem := item.Modified.GetMonitoredResourceDescriptor()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*monitored_resource_descriptor.MonitoredResourceDescriptorChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetMonitoredResourceDescriptorName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(monitored_resource_descriptor.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*monitored_resource_descriptor.MonitoredResourceDescriptor_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*monitored_resource_descriptor.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(monitored_resource_descriptor_client.NewMonitoredResourceDescriptorServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(monitored_resource_descriptor.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*monitored_resource_descriptor.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*monitored_resource_descriptor.ParentName)
		}
		return params
	})
}
