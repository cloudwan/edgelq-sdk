...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *time_series_forwarder_sink.TimeSeriesForwarderSink_FieldMask
	OrderBy   *time_series_forwarder_sink.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *time_series_forwarder_sink.ParentName
	Filter *time_series_forwarder_sink.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[time_series_forwarder_sink.Name]*time_series_forwarder_sink.TimeSeriesForwarderSink
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*time_series_forwarder_sink.TimeSeriesForwarderSinkChange
}

func NewWatcher(client time_series_forwarder_sink_client.TimeSeriesForwarderSinkServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &time_series_forwarder_sink.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &time_series_forwarder_sink.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "timeSeriesForwarderSink-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[time_series_forwarder_sink.Name]*time_series_forwarder_sink.TimeSeriesForwarderSink{}
...
type WatcherEventChange struct {
	pre, post *time_series_forwarder_sink.TimeSeriesForwarderSink
	name      *time_series_forwarder_sink.Name
}

func NewAddWatcherEventChange(resource *time_series_forwarder_sink.TimeSeriesForwarderSink) *WatcherEventChange {
...
func (c *WatcherEventChange) GetName() *time_series_forwarder_sink.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *time_series_forwarder_sink.TimeSeriesForwarderSink {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *time_series_forwarder_sink.TimeSeriesForwarderSink {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *time_series_forwarder_sink.TimeSeriesForwarderSink {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *time_series_forwarder_sink.TimeSeriesForwarderSink {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[time_series_forwarder_sink.Name]*time_series_forwarder_sink.TimeSeriesForwarderSink, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		timeSeriesForwarderSink := currentChange.GetCurrent()
		byName[*timeSeriesForwarderSink.GetName()] = timeSeriesForwarderSink
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			timeSeriesForwarderSink := change.GetCurrent()
			byName[*timeSeriesForwarderSink.GetName()] = timeSeriesForwarderSink
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, timeSeriesForwarderSink := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(timeSeriesForwarderSink))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[time_series_forwarder_sink.Name]*time_series_forwarder_sink.TimeSeriesForwarderSink{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *time_series_forwarder_sink.TimeSeriesForwarderSink
		switch item := change.ChangeType.(type) {
		case *time_series_forwarder_sink.TimeSeriesForwarderSinkChange_Added_:
			currentItem = item.Added.GetTimeSeriesForwarderSink()
		case *time_series_forwarder_sink.TimeSeriesForwarderSinkChange_Current_:
			currentItem = item.Current.GetTimeSeriesForwarderSink()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*time_series_forwarder_sink.TimeSeriesForwarderSinkChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *time_series_forwarder_sink.TimeSeriesForwarderSinkChange_Added_:
			newItem := item.Added.GetTimeSeriesForwarderSink()
		...
		case *time_series_forwarder_sink.TimeSeriesForwarderSinkChange_Modified_:
			updatedItem := item.Modified.GetTimeSeriesForwarderSink()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*time_series_forwarder_sink.TimeSeriesForwarderSinkChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetTimeSeriesForwarderSinkName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(time_series_forwarder_sink.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*time_series_forwarder_sink.TimeSeriesForwarderSink_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*time_series_forwarder_sink.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(time_series_forwarder_sink_client.NewTimeSeriesForwarderSinkServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(time_series_forwarder_sink.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*time_series_forwarder_sink.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*time_series_forwarder_sink.ParentName)
		}
		return params
	})
}
