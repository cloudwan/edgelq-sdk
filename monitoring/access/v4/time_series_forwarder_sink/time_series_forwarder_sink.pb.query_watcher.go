...
type QueryWatcherParams struct {
	...
	Cursor       *time_series_forwarder_sink.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*time_series_forwarder_sink.TimeSeriesForwarderSinkChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return time_series_forwarder_sink.TimeSeriesForwarderSinkChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client time_series_forwarder_sink_client.TimeSeriesForwarderSinkServiceClient,
...
func NewQueryWatcherWithIChan(id int, client time_series_forwarder_sink_client.TimeSeriesForwarderSinkServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "timeSeriesForwarderSink-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchTimeSeriesForwarderSinks(ctx, &time_series_forwarder_sink_client.WatchTimeSeriesForwarderSinksRequest{
		...
		if err != nil {
			...
			pending := make([]*time_series_forwarder_sink.TimeSeriesForwarderSinkChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetTimeSeriesForwarderSinkChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetTimeSeriesForwarderSinkChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(time_series_forwarder_sink.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(time_series_forwarder_sink.TimeSeriesForwarderSinkChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(time_series_forwarder_sink.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*time_series_forwarder_sink.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*time_series_forwarder_sink.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*time_series_forwarder_sink.Filter)
		}
		return NewQueryWatcherWithIChan(id, time_series_forwarder_sink_client.NewTimeSeriesForwarderSinkServiceClient(cc), cfg, ch)
	})
}
