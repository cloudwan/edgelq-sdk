...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return alert.AlertChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client alert_client.AlertServiceClient,
...
func NewQueryWatcherWithIChan(id int, client alert_client.AlertServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "alert-query-watcher").
	...
	for {
		stream, err := qw.client.WatchAlerts(ctx, &alert_client.WatchAlertsRequest{
		...
		if err != nil {
			...
			pending := make([]*alert.AlertChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetAlertChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(alert.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(alert.AlertChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(alert.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*alert.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*alert.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*alert.Filter)
		}
		return NewQueryWatcherWithIChan(id, alert_client.NewAlertServiceClient(cc), cfg, ch)
	})
}
