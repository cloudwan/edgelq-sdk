...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *notification_channel.NotificationChannel_FieldMask
	OrderBy   *notification_channel.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *notification_channel.ParentReference
	Filter *notification_channel.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[notification_channel.Name]*notification_channel.NotificationChannel
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*notification_channel.NotificationChannelChange
}

func NewWatcher(client notification_channel_client.NotificationChannelServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &notification_channel.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &notification_channel.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "notificationChannel-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[notification_channel.Name]*notification_channel.NotificationChannel{}
...
type WatcherEventChange struct {
	pre, post *notification_channel.NotificationChannel
	name      *notification_channel.Name
}

func NewAddWatcherEventChange(resource *notification_channel.NotificationChannel) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *notification_channel.NotificationChannel) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *notification_channel.NotificationChannel) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *notification_channel.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *notification_channel.NotificationChannel {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *notification_channel.NotificationChannel {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *notification_channel.NotificationChannel {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *notification_channel.NotificationChannel {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[notification_channel.Name]*notification_channel.NotificationChannel, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		notificationChannel := currentChange.GetCurrent()
		byName[*notificationChannel.GetName()] = notificationChannel
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			notificationChannel := change.GetCurrent()
			byName[*notificationChannel.GetName()] = notificationChannel
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, notificationChannel := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(notificationChannel))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[notification_channel.Name]*notification_channel.NotificationChannel{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *notification_channel.NotificationChannel
		switch item := change.ChangeType.(type) {
		case *notification_channel.NotificationChannelChange_Added_:
			currentItem = item.Added.GetNotificationChannel()
		case *notification_channel.NotificationChannelChange_Current_:
			currentItem = item.Current.GetNotificationChannel()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*notification_channel.NotificationChannelChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *notification_channel.NotificationChannelChange_Added_:
			newItem := item.Added.GetNotificationChannel()
		...
		case *notification_channel.NotificationChannelChange_Modified_:
			updatedItem := item.Modified.GetNotificationChannel()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*notification_channel.NotificationChannelChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetNotificationChannelName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(notification_channel.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*notification_channel.NotificationChannel_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*notification_channel.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(notification_channel_client.NewNotificationChannelServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(notification_channel.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*notification_channel.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*notification_channel.ParentReference)
		}
		return params
	})
}
