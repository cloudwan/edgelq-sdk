...
type QueryWatcherParams struct {
	...
	Cursor       *recovery_store_sharding_info.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*recovery_store_sharding_info.RecoveryStoreShardingInfoChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return recovery_store_sharding_info.RecoveryStoreShardingInfoChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client recovery_store_sharding_info_client.RecoveryStoreShardingInfoServiceClient,
...
func NewQueryWatcherWithIChan(id int, client recovery_store_sharding_info_client.RecoveryStoreShardingInfoServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "recoveryStoreShardingInfo-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchRecoveryStoreShardingInfos(ctx, &recovery_store_sharding_info_client.WatchRecoveryStoreShardingInfosRequest{
		...
		if err != nil {
			...
			pending := make([]*recovery_store_sharding_info.RecoveryStoreShardingInfoChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetRecoveryStoreShardingInfoChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetRecoveryStoreShardingInfoChanges(),
...
func init() {
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(recovery_store_sharding_info.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		return NewQueryWatcherWithIChan(id, recovery_store_sharding_info_client.NewRecoveryStoreShardingInfoServiceClient(cc), cfg, ch)
	})
}
