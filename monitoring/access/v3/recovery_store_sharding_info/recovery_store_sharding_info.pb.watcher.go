...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &recovery_store_sharding_info.ParentName{}
...
func init() {
	...
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(recovery_store_sharding_info.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*recovery_store_sharding_info.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*recovery_store_sharding_info.ParentName)
		}
		return params
	})
}
