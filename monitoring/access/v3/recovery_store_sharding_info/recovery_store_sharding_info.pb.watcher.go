...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	OrderBy   *recovery_store_sharding_info.OrderBy
	ChunkSize int
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[recovery_store_sharding_info.Name]*recovery_store_sharding_info.RecoveryStoreShardingInfo
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*recovery_store_sharding_info.RecoveryStoreShardingInfoChange
}

func NewWatcher(client recovery_store_sharding_info_client.RecoveryStoreShardingInfoServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &recovery_store_sharding_info.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &recovery_store_sharding_info.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "recoveryStoreShardingInfo-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[recovery_store_sharding_info.Name]*recovery_store_sharding_info.RecoveryStoreShardingInfo{}
...
type WatcherEventChange struct {
	pre, post *recovery_store_sharding_info.RecoveryStoreShardingInfo
	name      *recovery_store_sharding_info.Name
}

func NewAddWatcherEventChange(resource *recovery_store_sharding_info.RecoveryStoreShardingInfo) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *recovery_store_sharding_info.RecoveryStoreShardingInfo) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *recovery_store_sharding_info.RecoveryStoreShardingInfo) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *recovery_store_sharding_info.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *recovery_store_sharding_info.RecoveryStoreShardingInfo {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *recovery_store_sharding_info.RecoveryStoreShardingInfo {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *recovery_store_sharding_info.RecoveryStoreShardingInfo {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *recovery_store_sharding_info.RecoveryStoreShardingInfo {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[recovery_store_sharding_info.Name]*recovery_store_sharding_info.RecoveryStoreShardingInfo, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		recoveryStoreShardingInfo := currentChange.GetCurrent()
		byName[*recoveryStoreShardingInfo.GetName()] = recoveryStoreShardingInfo
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			recoveryStoreShardingInfo := change.GetCurrent()
			byName[*recoveryStoreShardingInfo.GetName()] = recoveryStoreShardingInfo
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, recoveryStoreShardingInfo := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(recoveryStoreShardingInfo))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[recovery_store_sharding_info.Name]*recovery_store_sharding_info.RecoveryStoreShardingInfo{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *recovery_store_sharding_info.RecoveryStoreShardingInfo
		switch item := change.ChangeType.(type) {
		case *recovery_store_sharding_info.RecoveryStoreShardingInfoChange_Added_:
			currentItem = item.Added.GetRecoveryStoreShardingInfo()
		case *recovery_store_sharding_info.RecoveryStoreShardingInfoChange_Current_:
			currentItem = item.Current.GetRecoveryStoreShardingInfo()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*recovery_store_sharding_info.RecoveryStoreShardingInfoChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *recovery_store_sharding_info.RecoveryStoreShardingInfoChange_Added_:
			newItem := item.Added.GetRecoveryStoreShardingInfo()
		...
		case *recovery_store_sharding_info.RecoveryStoreShardingInfoChange_Modified_:
			updatedItem := item.Modified.GetRecoveryStoreShardingInfo()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*recovery_store_sharding_info.RecoveryStoreShardingInfoChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetRecoveryStoreShardingInfoName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(recovery_store_sharding_info.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*recovery_store_sharding_info.RecoveryStoreShardingInfo_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*recovery_store_sharding_info.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(recovery_store_sharding_info_client.NewRecoveryStoreShardingInfoServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(recovery_store_sharding_info.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*recovery_store_sharding_info.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*recovery_store_sharding_info.ParentReference)
		}
		return params
	})
}
