...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	OrderBy   *alerting_condition.OrderBy
	ChunkSize int
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[alerting_condition.Name]*alerting_condition.AlertingCondition
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*alerting_condition.AlertingConditionChange
}

func NewWatcher(client alerting_condition_client.AlertingConditionServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &alerting_condition.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &alerting_condition.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "alertingCondition-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[alerting_condition.Name]*alerting_condition.AlertingCondition{}
...
type WatcherEventChange struct {
	pre, post *alerting_condition.AlertingCondition
	name      *alerting_condition.Name
}

func NewAddWatcherEventChange(resource *alerting_condition.AlertingCondition) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *alerting_condition.AlertingCondition) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *alerting_condition.AlertingCondition) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *alerting_condition.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *alerting_condition.AlertingCondition {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *alerting_condition.AlertingCondition {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *alerting_condition.AlertingCondition {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *alerting_condition.AlertingCondition {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[alerting_condition.Name]*alerting_condition.AlertingCondition, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		alertingCondition := currentChange.GetCurrent()
		byName[*alertingCondition.GetName()] = alertingCondition
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			alertingCondition := change.GetCurrent()
			byName[*alertingCondition.GetName()] = alertingCondition
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, alertingCondition := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(alertingCondition))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[alerting_condition.Name]*alerting_condition.AlertingCondition{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *alerting_condition.AlertingCondition
		switch item := change.ChangeType.(type) {
		case *alerting_condition.AlertingConditionChange_Added_:
			currentItem = item.Added.GetAlertingCondition()
		case *alerting_condition.AlertingConditionChange_Current_:
			currentItem = item.Current.GetAlertingCondition()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*alerting_condition.AlertingConditionChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *alerting_condition.AlertingConditionChange_Added_:
			newItem := item.Added.GetAlertingCondition()
		...
		case *alerting_condition.AlertingConditionChange_Modified_:
			updatedItem := item.Modified.GetAlertingCondition()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*alerting_condition.AlertingConditionChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetAlertingConditionName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(alerting_condition.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*alerting_condition.AlertingCondition_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*alerting_condition.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(alerting_condition_client.NewAlertingConditionServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(alerting_condition.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*alerting_condition.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*alerting_condition.ParentReference)
		}
		return params
	})
}
