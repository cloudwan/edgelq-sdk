...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *metric_descriptor.MetricDescriptor_FieldMask
	OrderBy   *metric_descriptor.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *metric_descriptor.ParentReference
	Filter *metric_descriptor.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[metric_descriptor.Name]*metric_descriptor.MetricDescriptor
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*metric_descriptor.MetricDescriptorChange
}

func NewWatcher(client metric_descriptor_client.MetricDescriptorServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &metric_descriptor.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &metric_descriptor.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "metricDescriptor-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[metric_descriptor.Name]*metric_descriptor.MetricDescriptor{}
...
type WatcherEventChange struct {
	pre, post *metric_descriptor.MetricDescriptor
	name      *metric_descriptor.Name
}

func NewAddWatcherEventChange(resource *metric_descriptor.MetricDescriptor) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *metric_descriptor.MetricDescriptor) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *metric_descriptor.MetricDescriptor) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *metric_descriptor.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *metric_descriptor.MetricDescriptor {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *metric_descriptor.MetricDescriptor {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *metric_descriptor.MetricDescriptor {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *metric_descriptor.MetricDescriptor {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[metric_descriptor.Name]*metric_descriptor.MetricDescriptor, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		metricDescriptor := currentChange.GetCurrent()
		byName[*metricDescriptor.GetName()] = metricDescriptor
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			metricDescriptor := change.GetCurrent()
			byName[*metricDescriptor.GetName()] = metricDescriptor
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, metricDescriptor := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(metricDescriptor))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[metric_descriptor.Name]*metric_descriptor.MetricDescriptor{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *metric_descriptor.MetricDescriptor
		switch item := change.ChangeType.(type) {
		case *metric_descriptor.MetricDescriptorChange_Added_:
			currentItem = item.Added.GetMetricDescriptor()
		case *metric_descriptor.MetricDescriptorChange_Current_:
			currentItem = item.Current.GetMetricDescriptor()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*metric_descriptor.MetricDescriptorChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *metric_descriptor.MetricDescriptorChange_Added_:
			newItem := item.Added.GetMetricDescriptor()
		...
		case *metric_descriptor.MetricDescriptorChange_Modified_:
			updatedItem := item.Modified.GetMetricDescriptor()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*metric_descriptor.MetricDescriptorChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range pendingChanges {
		name := *pendingChange.GetMetricDescriptorName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(metric_descriptor.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*metric_descriptor.MetricDescriptor_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*metric_descriptor.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(metric_descriptor_client.NewMetricDescriptorServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(metric_descriptor.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*metric_descriptor.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*metric_descriptor.ParentReference)
		}
		return params
	})
}
