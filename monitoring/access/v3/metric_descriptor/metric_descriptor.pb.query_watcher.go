...
type QueryWatcherParams struct {
	...
	Cursor       *metric_descriptor.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*metric_descriptor.MetricDescriptorChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return metric_descriptor.MetricDescriptorChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client metric_descriptor_client.MetricDescriptorServiceClient,
...
func NewQueryWatcherWithIChan(id int, client metric_descriptor_client.MetricDescriptorServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "metricDescriptor-query-watcher").
	...
	for {
		stream, err := qw.client.WatchMetricDescriptors(ctx, &metric_descriptor_client.WatchMetricDescriptorsRequest{
		...
		if err != nil {
			...
			pending := make([]*metric_descriptor.MetricDescriptorChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetMetricDescriptorChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetMetricDescriptorChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(metric_descriptor.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(metric_descriptor.MetricDescriptorChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(metric_descriptor.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*metric_descriptor.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*metric_descriptor.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*metric_descriptor.Filter)
		}
		return NewQueryWatcherWithIChan(id, metric_descriptor_client.NewMetricDescriptorServiceClient(cc), cfg, ch)
	})
}
