...
type WatcherFilterParams struct {
	Parent *notification.ParentReference
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[notification.Name]*notification.Notification
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*notification.NotificationChange
}

func NewWatcher(client notification_client.NotificationServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &notification.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &notification.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "notification-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[notification.Name]*notification.Notification{}
...
type WatcherEventChange struct {
	pre, post *notification.Notification
...
func (c *WatcherEventChange) GetAdded() *notification.Notification {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[notification.Name]*notification.Notification, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		notification := currentChange.GetCurrent()
		byName[*notification.GetName()] = notification
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			notification := change.GetCurrent()
			byName[*notification.GetName()] = notification
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, notification := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(notification))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[notification.Name]*notification.Notification{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *notification.Notification
		switch item := change.ChangeType.(type) {
		case *notification.NotificationChange_Added_:
			currentItem = item.Added.GetNotification()
		case *notification.NotificationChange_Current_:
			currentItem = item.Current.GetNotification()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*notification.NotificationChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *notification.NotificationChange_Added_:
			newItem := item.Added.GetNotification()
		...
		case *notification.NotificationChange_Modified_:
			updatedItem := item.Modified.GetNotification()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*notification.NotificationChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetNotificationName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(notification.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*notification.Notification_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*notification.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(notification_client.NewNotificationServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(notification.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*notification.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*notification.ParentReference)
		}
		return params
	})
}
