...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return notification.NotificationChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client notification_client.NotificationServiceClient,
...
func NewQueryWatcherWithIChan(id int, client notification_client.NotificationServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "notification-query-watcher").
	...
	for {
		stream, err := qw.client.WatchNotifications(ctx, &notification_client.WatchNotificationsRequest{
		...
		if err != nil {
			...
			pending := make([]*notification.NotificationChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetNotificationChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetNotificationChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(notification.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(notification.NotificationChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(notification.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*notification.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*notification.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*notification.Filter)
		}
		return NewQueryWatcherWithIChan(id, notification_client.NewNotificationServiceClient(cc), cfg, ch)
	})
}
