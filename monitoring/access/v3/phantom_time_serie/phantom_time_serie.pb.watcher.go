...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *phantom_time_serie.PhantomTimeSerie_FieldMask
	OrderBy   *phantom_time_serie.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *phantom_time_serie.ParentReference
	Filter *phantom_time_serie.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[phantom_time_serie.Name]*phantom_time_serie.PhantomTimeSerie
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*phantom_time_serie.PhantomTimeSerieChange
}

func NewWatcher(client phantom_time_serie_client.PhantomTimeSerieServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &phantom_time_serie.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &phantom_time_serie.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "phantomTimeSerie-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[phantom_time_serie.Name]*phantom_time_serie.PhantomTimeSerie{}
...
type WatcherEventChange struct {
	pre, post *phantom_time_serie.PhantomTimeSerie
	name      *phantom_time_serie.Name
}

func NewAddWatcherEventChange(resource *phantom_time_serie.PhantomTimeSerie) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *phantom_time_serie.PhantomTimeSerie) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *phantom_time_serie.PhantomTimeSerie) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *phantom_time_serie.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *phantom_time_serie.PhantomTimeSerie {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *phantom_time_serie.PhantomTimeSerie {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *phantom_time_serie.PhantomTimeSerie {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *phantom_time_serie.PhantomTimeSerie {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[phantom_time_serie.Name]*phantom_time_serie.PhantomTimeSerie, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		phantomTimeSerie := currentChange.GetCurrent()
		byName[*phantomTimeSerie.GetName()] = phantomTimeSerie
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			phantomTimeSerie := change.GetCurrent()
			byName[*phantomTimeSerie.GetName()] = phantomTimeSerie
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, phantomTimeSerie := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(phantomTimeSerie))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[phantom_time_serie.Name]*phantom_time_serie.PhantomTimeSerie{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *phantom_time_serie.PhantomTimeSerie
		switch item := change.ChangeType.(type) {
		case *phantom_time_serie.PhantomTimeSerieChange_Added_:
			currentItem = item.Added.GetPhantomTimeSerie()
		case *phantom_time_serie.PhantomTimeSerieChange_Current_:
			currentItem = item.Current.GetPhantomTimeSerie()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*phantom_time_serie.PhantomTimeSerieChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *phantom_time_serie.PhantomTimeSerieChange_Added_:
			newItem := item.Added.GetPhantomTimeSerie()
		...
		case *phantom_time_serie.PhantomTimeSerieChange_Modified_:
			updatedItem := item.Modified.GetPhantomTimeSerie()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*phantom_time_serie.PhantomTimeSerieChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetPhantomTimeSerieName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(phantom_time_serie.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*phantom_time_serie.PhantomTimeSerie_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*phantom_time_serie.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(phantom_time_serie_client.NewPhantomTimeSerieServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(phantom_time_serie.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*phantom_time_serie.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*phantom_time_serie.ParentReference)
		}
		return params
	})
}
