...
type QueryWatcherParams struct {
	...
	Cursor       *role_binding.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*role_binding.RoleBindingChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return role_binding.RoleBindingChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client role_binding_client.RoleBindingServiceClient,
...
func NewQueryWatcherWithIChan(id int, client role_binding_client.RoleBindingServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "roleBinding-query-watcher").
	...
	for {
		stream, err := qw.client.WatchRoleBindings(ctx, &role_binding_client.WatchRoleBindingsRequest{
		...
		if err != nil {
			...
			pending := make([]*role_binding.RoleBindingChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetRoleBindingChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetRoleBindingChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(role_binding.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		return NewQueryWatcherWithIChan(id, role_binding_client.NewRoleBindingServiceClient(cc), cfg, ch)
	})
}
