...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *project_invitation.ProjectInvitation_FieldMask
	OrderBy   *project_invitation.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *project_invitation.ParentReference
	Filter *project_invitation.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[project_invitation.Name]*project_invitation.ProjectInvitation
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*project_invitation.ProjectInvitationChange
}

func NewWatcher(client project_invitation_client.ProjectInvitationServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &project_invitation.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &project_invitation.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "projectInvitation-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[project_invitation.Name]*project_invitation.ProjectInvitation{}
...
type WatcherEventChange struct {
	pre, post *project_invitation.ProjectInvitation
	name      *project_invitation.Name
}

func NewAddWatcherEventChange(resource *project_invitation.ProjectInvitation) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *project_invitation.ProjectInvitation) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *project_invitation.ProjectInvitation) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *project_invitation.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *project_invitation.ProjectInvitation {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *project_invitation.ProjectInvitation {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *project_invitation.ProjectInvitation {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *project_invitation.ProjectInvitation {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[project_invitation.Name]*project_invitation.ProjectInvitation, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		projectInvitation := currentChange.GetCurrent()
		byName[*projectInvitation.GetName()] = projectInvitation
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			projectInvitation := change.GetCurrent()
			byName[*projectInvitation.GetName()] = projectInvitation
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, projectInvitation := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(projectInvitation))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[project_invitation.Name]*project_invitation.ProjectInvitation{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *project_invitation.ProjectInvitation
		switch item := change.ChangeType.(type) {
		case *project_invitation.ProjectInvitationChange_Added_:
			currentItem = item.Added.GetProjectInvitation()
		case *project_invitation.ProjectInvitationChange_Current_:
			currentItem = item.Current.GetProjectInvitation()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*project_invitation.ProjectInvitationChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *project_invitation.ProjectInvitationChange_Added_:
			newItem := item.Added.GetProjectInvitation()
		...
		case *project_invitation.ProjectInvitationChange_Modified_:
			updatedItem := item.Modified.GetProjectInvitation()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*project_invitation.ProjectInvitationChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetProjectInvitationName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(project_invitation.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*project_invitation.ProjectInvitation_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*project_invitation.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(project_invitation_client.NewProjectInvitationServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(project_invitation.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*project_invitation.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*project_invitation.ParentReference)
		}
		return params
	})
}
