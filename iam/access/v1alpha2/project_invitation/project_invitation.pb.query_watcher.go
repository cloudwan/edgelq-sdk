...
type QueryWatcherParams struct {
	...
	Cursor       *project_invitation.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*project_invitation.ProjectInvitationChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return project_invitation.ProjectInvitationChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client project_invitation_client.ProjectInvitationServiceClient,
...
func NewQueryWatcherWithIChan(id int, client project_invitation_client.ProjectInvitationServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "projectInvitation-query-watcher").
	...
	for {
		stream, err := qw.client.WatchProjectInvitations(ctx, &project_invitation_client.WatchProjectInvitationsRequest{
		...
		if err != nil {
			...
			pending := make([]*project_invitation.ProjectInvitationChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetProjectInvitationChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetProjectInvitationChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(project_invitation.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(project_invitation.ProjectInvitationChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(project_invitation.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*project_invitation.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*project_invitation.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*project_invitation.Filter)
		}
		return NewQueryWatcherWithIChan(id, project_invitation_client.NewProjectInvitationServiceClient(cc), cfg, ch)
	})
}
