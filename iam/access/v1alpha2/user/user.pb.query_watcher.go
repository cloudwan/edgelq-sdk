...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return user.UserChangeList(e.Changes)
}
...
func NewQueryWatcherWithIChan(id int, client user_client.UserServiceClient,
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(user.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(user.UserChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(user.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*user.PagerCursor)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*user.Filter)
		}
		return NewQueryWatcherWithIChan(id, user_client.NewUserServiceClient(cc), cfg, ch)
	})
}
