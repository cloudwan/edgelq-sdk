...
type QueryWatcherParams struct {
	Parent       *service_account_key.ParentName
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*service_account_key.ServiceAccountKeyChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "serviceAccountKey-query-watcher").
			...
			for {
				...
				if err != nil {
					...
					if inSync {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetServiceAccountKeyChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(service_account_key.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(service_account_key.ServiceAccountKeyChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(service_account_key.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*service_account_key.ParentName)
		}
