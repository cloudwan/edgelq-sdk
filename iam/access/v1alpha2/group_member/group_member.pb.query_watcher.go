...
type QueryWatcherParams struct {
	...
	Cursor       *group_member.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*group_member.GroupMemberChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return group_member.GroupMemberChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client group_member_client.GroupMemberServiceClient,
...
func NewQueryWatcherWithIChan(id int, client group_member_client.GroupMemberServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "groupMember-query-watcher").
	...
	for {
		stream, err := qw.client.WatchGroupMembers(ctx, &group_member_client.WatchGroupMembersRequest{
		...
		if err != nil {
			...
			pending := make([]*group_member.GroupMemberChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetGroupMemberChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetGroupMemberChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(group_member.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(group_member.GroupMemberChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(group_member.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*group_member.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*group_member.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*group_member.Filter)
		}
		return NewQueryWatcherWithIChan(id, group_member_client.NewGroupMemberServiceClient(cc), cfg, ch)
	})
}
