...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *service_account.ServiceAccount_FieldMask
	OrderBy   *service_account.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *service_account.ParentReference
	Filter *service_account.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[service_account.Name]*service_account.ServiceAccount
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*service_account.ServiceAccountChange
}

func NewWatcher(client service_account_client.ServiceAccountServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &service_account.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &service_account.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "serviceAccount-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[service_account.Name]*service_account.ServiceAccount{}
...
type WatcherEventChange struct {
	pre, post *service_account.ServiceAccount
	name      *service_account.Name
}

func NewAddWatcherEventChange(resource *service_account.ServiceAccount) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *service_account.ServiceAccount) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *service_account.ServiceAccount) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *service_account.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *service_account.ServiceAccount {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *service_account.ServiceAccount {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *service_account.ServiceAccount {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *service_account.ServiceAccount {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[service_account.Name]*service_account.ServiceAccount, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		serviceAccount := currentChange.GetCurrent()
		byName[*serviceAccount.GetName()] = serviceAccount
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			serviceAccount := change.GetCurrent()
			byName[*serviceAccount.GetName()] = serviceAccount
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, serviceAccount := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(serviceAccount))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[service_account.Name]*service_account.ServiceAccount{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *service_account.ServiceAccount
		switch item := change.ChangeType.(type) {
		case *service_account.ServiceAccountChange_Added_:
			currentItem = item.Added.GetServiceAccount()
		case *service_account.ServiceAccountChange_Current_:
			currentItem = item.Current.GetServiceAccount()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*service_account.ServiceAccountChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *service_account.ServiceAccountChange_Added_:
			newItem := item.Added.GetServiceAccount()
		...
		case *service_account.ServiceAccountChange_Modified_:
			updatedItem := item.Modified.GetServiceAccount()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*service_account.ServiceAccountChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range pendingChanges {
		name := *pendingChange.GetServiceAccountName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(service_account.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*service_account.ServiceAccount_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*service_account.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(service_account_client.NewServiceAccountServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(service_account.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*service_account.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*service_account.ParentReference)
		}
		return params
	})
}
