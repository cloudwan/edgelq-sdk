...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *organization.Organization_FieldMask
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[organization.Name]*organization.Organization
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*organization.OrganizationChange
}

func NewWatcher(client organization_client.OrganizationServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Filter != nil {
			...
			cQuery.Filter = &organization.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "organization-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[organization.Name]*organization.Organization{}
...
type WatcherEventChange struct {
	pre, post *organization.Organization
	name      *organization.Name
}

func NewAddWatcherEventChange(resource *organization.Organization) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *organization.Organization) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *organization.Organization) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *organization.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *organization.Organization {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *organization.Organization {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *organization.Organization {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *organization.Organization {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[organization.Name]*organization.Organization, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		organization := currentChange.GetCurrent()
	...
	for _, change := range src.Changes {
		if change.IsDelete() {
			...
			organization := change.GetCurrent()
			byName[*organization.GetName()] = organization
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, organization := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(organization))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[organization.Name]*organization.Organization{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *organization.Organization
		switch item := change.ChangeType.(type) {
		case *organization.OrganizationChange_Added_:
			currentItem = item.Added.GetOrganization()
		case *organization.OrganizationChange_Current_:
			currentItem = item.Current.GetOrganization()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*organization.OrganizationChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *organization.OrganizationChange_Added_:
			newItem := item.Added.GetOrganization()
		...
		case *organization.OrganizationChange_Modified_:
			updatedItem := item.Modified.GetOrganization()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*organization.OrganizationChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetOrganizationName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(organization.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*organization.Organization_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*organization.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(organization_client.NewOrganizationServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(organization.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*organization.Filter)
		}
		return params
	})
}
