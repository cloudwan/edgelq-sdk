...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return group.GroupChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client group_client.GroupServiceClient,
...
func NewQueryWatcherWithIChan(id int, client group_client.GroupServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	...
	for {
		stream, err := qw.client.WatchGroups(ctx, &group_client.WatchGroupsRequest{
		...
		if err != nil {
			...
			pending := make([]*group.GroupChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetGroupChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(group.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(group.GroupChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(group.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*group.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*group.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*group.Filter)
		}
		return NewQueryWatcherWithIChan(id, group_client.NewGroupServiceClient(cc), cfg, ch)
	})
}
