...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return condition.ConditionChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client condition_client.ConditionServiceClient,
...
func NewQueryWatcherWithIChan(id int, client condition_client.ConditionServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "condition-query-watcher").
	...
	for {
		stream, err := qw.client.WatchConditions(ctx, &condition_client.WatchConditionsRequest{
		...
		if err != nil {
			...
			pending := make([]*condition.ConditionChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetConditionChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetConditionChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(condition.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(condition.ConditionChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(condition.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*condition.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*condition.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*condition.Filter)
		}
		return NewQueryWatcherWithIChan(id, condition_client.NewConditionServiceClient(cc), cfg, ch)
	})
}
