...
type WatcherFilterParams struct {
	Parent *condition.ParentReference
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[condition.Name]*condition.Condition
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*condition.ConditionChange
}

func NewWatcher(client condition_client.ConditionServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &condition.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &condition.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "condition-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[condition.Name]*condition.Condition{}
...
type WatcherEventChange struct {
	pre, post *condition.Condition
	name      *condition.Name
}

func NewAddWatcherEventChange(resource *condition.Condition) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *condition.Condition) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *condition.Condition) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *condition.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *condition.Condition {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *condition.Condition {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *condition.Condition {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *condition.Condition {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[condition.Name]*condition.Condition, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		condition := currentChange.GetCurrent()
		byName[*condition.GetName()] = condition
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			condition := change.GetCurrent()
			byName[*condition.GetName()] = condition
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, condition := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(condition))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[condition.Name]*condition.Condition{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *condition.Condition
		switch item := change.ChangeType.(type) {
		case *condition.ConditionChange_Added_:
			currentItem = item.Added.GetCondition()
		case *condition.ConditionChange_Current_:
			currentItem = item.Current.GetCondition()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*condition.ConditionChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *condition.ConditionChange_Added_:
			newItem := item.Added.GetCondition()
		...
		case *condition.ConditionChange_Modified_:
			updatedItem := item.Modified.GetCondition()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*condition.ConditionChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetConditionName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(condition.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*condition.Condition_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*condition.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(condition_client.NewConditionServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(condition.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*condition.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*condition.ParentReference)
		}
		return params
	})
}
