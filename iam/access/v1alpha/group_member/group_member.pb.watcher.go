...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *group_member.GroupMember_FieldMask
	OrderBy   *group_member.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *group_member.ParentReference
	Filter *group_member.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[group_member.Name]*group_member.GroupMember
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*group_member.GroupMemberChange
}

func NewWatcher(client group_member_client.GroupMemberServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &group_member.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &group_member.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "groupMember-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[group_member.Name]*group_member.GroupMember{}
...
type WatcherEventChange struct {
	pre, post *group_member.GroupMember
	name      *group_member.Name
}

func NewAddWatcherEventChange(resource *group_member.GroupMember) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *group_member.GroupMember) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *group_member.GroupMember) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *group_member.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *group_member.GroupMember {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *group_member.GroupMember {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *group_member.GroupMember {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *group_member.GroupMember {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[group_member.Name]*group_member.GroupMember, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		groupMember := currentChange.GetCurrent()
		byName[*groupMember.GetName()] = groupMember
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			groupMember := change.GetCurrent()
			byName[*groupMember.GetName()] = groupMember
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, groupMember := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(groupMember))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[group_member.Name]*group_member.GroupMember{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *group_member.GroupMember
		switch item := change.ChangeType.(type) {
		case *group_member.GroupMemberChange_Added_:
			currentItem = item.Added.GetGroupMember()
		case *group_member.GroupMemberChange_Current_:
			currentItem = item.Current.GetGroupMember()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*group_member.GroupMemberChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *group_member.GroupMemberChange_Added_:
			newItem := item.Added.GetGroupMember()
		...
		case *group_member.GroupMemberChange_Modified_:
			updatedItem := item.Modified.GetGroupMember()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*group_member.GroupMemberChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetGroupMemberName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(group_member.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*group_member.GroupMember_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*group_member.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(group_member_client.NewGroupMemberServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(group_member.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*group_member.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*group_member.ParentReference)
		}
		return params
	})
}
