...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[role_binding.Name]*role_binding.RoleBinding
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*role_binding.RoleBindingChange
}

func NewWatcher(client role_binding_client.RoleBindingServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &role_binding.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &role_binding.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "roleBinding-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[role_binding.Name]*role_binding.RoleBinding{}
...
type WatcherEventChange struct {
	pre, post *role_binding.RoleBinding
	name      *role_binding.Name
}

func NewAddWatcherEventChange(resource *role_binding.RoleBinding) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *role_binding.RoleBinding) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *role_binding.RoleBinding) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *role_binding.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *role_binding.RoleBinding {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *role_binding.RoleBinding {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *role_binding.RoleBinding {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *role_binding.RoleBinding {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[role_binding.Name]*role_binding.RoleBinding, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		roleBinding := currentChange.GetCurrent()
		byName[*roleBinding.GetName()] = roleBinding
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			roleBinding := change.GetCurrent()
			byName[*roleBinding.GetName()] = roleBinding
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, roleBinding := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(roleBinding))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[role_binding.Name]*role_binding.RoleBinding{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *role_binding.RoleBinding
		switch item := change.ChangeType.(type) {
		case *role_binding.RoleBindingChange_Added_:
			currentItem = item.Added.GetRoleBinding()
		case *role_binding.RoleBindingChange_Current_:
			currentItem = item.Current.GetRoleBinding()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*role_binding.RoleBindingChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *role_binding.RoleBindingChange_Added_:
			newItem := item.Added.GetRoleBinding()
		...
		case *role_binding.RoleBindingChange_Modified_:
			updatedItem := item.Modified.GetRoleBinding()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*role_binding.RoleBindingChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetRoleBindingName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(role_binding.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*role_binding.RoleBinding_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*role_binding.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(role_binding_client.NewRoleBindingServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(role_binding.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*role_binding.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*role_binding.ParentReference)
		}
		return params
	})
}
