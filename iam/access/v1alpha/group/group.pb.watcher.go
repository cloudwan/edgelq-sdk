...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *group.Group_FieldMask
	OrderBy   *group.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *group.ParentReference
	Filter *group.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[group.Name]*group.Group
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*group.GroupChange
}

func NewWatcher(client group_client.GroupServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &group.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &group.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "group-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[group.Name]*group.Group{}
...
type WatcherEventChange struct {
	pre, post *group.Group
	name      *group.Name
}

func NewAddWatcherEventChange(resource *group.Group) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *group.Group) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *group.Group) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *group.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *group.Group {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *group.Group {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *group.Group {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *group.Group {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[group.Name]*group.Group, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		group := currentChange.GetCurrent()
	...
	for _, change := range src.Changes {
		if change.IsDelete() {
			...
			group := change.GetCurrent()
			byName[*group.GetName()] = group
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, group := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(group))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*group.GroupChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *group.GroupChange_Added_:
			newItem := item.Added.GetGroup()
		...
		case *group.GroupChange_Modified_:
			updatedItem := item.Modified.GetGroup()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*group.GroupChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetGroupName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(group.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*group.Group_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*group.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(group_client.NewGroupServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(group.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*group.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*group.ParentReference)
		}
		return params
	})
}
