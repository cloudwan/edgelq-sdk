...
type QueryWatcherParams struct {
	...
	Cursor       *service_account.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*service_account.ServiceAccountChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return service_account.ServiceAccountChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client service_account_client.ServiceAccountServiceClient,
...
func NewQueryWatcherWithIChan(id int, client service_account_client.ServiceAccountServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "serviceAccount-query-watcher").
	...
	for {
		stream, err := qw.client.WatchServiceAccounts(ctx, &service_account_client.WatchServiceAccountsRequest{
		...
		if err != nil {
			...
			pending := make([]*service_account.ServiceAccountChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetServiceAccountChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetServiceAccountChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(service_account.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(service_account.ServiceAccountChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(service_account.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*service_account.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*service_account.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*service_account.Filter)
		}
		return NewQueryWatcherWithIChan(id, service_account_client.NewServiceAccountServiceClient(cc), cfg, ch)
	})
}
