...
type QueryWatcherParams struct {
	...
	Cursor       *organization_invitation.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*organization_invitation.OrganizationInvitationChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return organization_invitation.OrganizationInvitationChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client organization_invitation_client.OrganizationInvitationServiceClient,
...
func NewQueryWatcherWithIChan(id int, client organization_invitation_client.OrganizationInvitationServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "organizationInvitation-query-watcher").
	...
	for {
		stream, err := qw.client.WatchOrganizationInvitations(ctx, &organization_invitation_client.WatchOrganizationInvitationsRequest{
		...
		if err != nil {
			...
			pending := make([]*organization_invitation.OrganizationInvitationChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetOrganizationInvitationChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetOrganizationInvitationChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(organization_invitation.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(organization_invitation.OrganizationInvitationChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(organization_invitation.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*organization_invitation.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*organization_invitation.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*organization_invitation.Filter)
		}
		return NewQueryWatcherWithIChan(id, organization_invitation_client.NewOrganizationInvitationServiceClient(cc), cfg, ch)
	})
}
