...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return role.RoleChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client role_client.RoleServiceClient,
...
func NewQueryWatcherWithIChan(id int, client role_client.RoleServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "role-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchRoles(ctx, &role_client.WatchRolesRequest{
		...
		if err != nil {
			...
			pending := make([]*role.RoleChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetRoleChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(role.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(role.RoleChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(role.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*role.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*role.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*role.Filter)
		}
		return NewQueryWatcherWithIChan(id, role_client.NewRoleServiceClient(cc), cfg, ch)
	})
}
