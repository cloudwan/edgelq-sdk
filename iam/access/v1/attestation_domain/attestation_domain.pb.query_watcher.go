...
type QueryWatcherParams struct {
	...
	Cursor       *attestation_domain.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*attestation_domain.AttestationDomainChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return attestation_domain.AttestationDomainChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client attestation_domain_client.AttestationDomainServiceClient,
...
func NewQueryWatcherWithIChan(id int, client attestation_domain_client.AttestationDomainServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "attestationDomain-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchAttestationDomains(ctx, &attestation_domain_client.WatchAttestationDomainsRequest{
		...
		if err != nil {
			...
			pending := make([]*attestation_domain.AttestationDomainChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetAttestationDomainChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetAttestationDomainChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(attestation_domain.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(attestation_domain.AttestationDomainChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(attestation_domain.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*attestation_domain.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*attestation_domain.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*attestation_domain.Filter)
		}
		return NewQueryWatcherWithIChan(id, attestation_domain_client.NewAttestationDomainServiceClient(cc), cfg, ch)
	})
}
