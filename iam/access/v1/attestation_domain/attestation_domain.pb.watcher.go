...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *attestation_domain.AttestationDomain_FieldMask
	OrderBy   *attestation_domain.OrderBy
	ChunkSize int
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[attestation_domain.Name]*attestation_domain.AttestationDomain
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*attestation_domain.AttestationDomainChange
}
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Filter != nil {
			...
			cQuery.Filter = &attestation_domain.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "attestationDomain-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[attestation_domain.Name]*attestation_domain.AttestationDomain{}
...
type WatcherEventChange struct {
	pre, post *attestation_domain.AttestationDomain
	name      *attestation_domain.Name
}

func NewAddWatcherEventChange(resource *attestation_domain.AttestationDomain) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *attestation_domain.AttestationDomain) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *attestation_domain.AttestationDomain) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *attestation_domain.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *attestation_domain.AttestationDomain {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *attestation_domain.AttestationDomain {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *attestation_domain.AttestationDomain {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *attestation_domain.AttestationDomain {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[attestation_domain.Name]*attestation_domain.AttestationDomain, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		attestationDomain := currentChange.GetCurrent()
		byName[*attestationDomain.GetName()] = attestationDomain
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			attestationDomain := change.GetCurrent()
			byName[*attestationDomain.GetName()] = attestationDomain
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, attestationDomain := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(attestationDomain))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[attestation_domain.Name]*attestation_domain.AttestationDomain{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *attestation_domain.AttestationDomain
		switch item := change.ChangeType.(type) {
		case *attestation_domain.AttestationDomainChange_Added_:
			currentItem = item.Added.GetAttestationDomain()
		case *attestation_domain.AttestationDomainChange_Current_:
			currentItem = item.Current.GetAttestationDomain()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*attestation_domain.AttestationDomainChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *attestation_domain.AttestationDomainChange_Added_:
			newItem := item.Added.GetAttestationDomain()
		...
		case *attestation_domain.AttestationDomainChange_Modified_:
			updatedItem := item.Modified.GetAttestationDomain()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*attestation_domain.AttestationDomainChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetAttestationDomainName()
