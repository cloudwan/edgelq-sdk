...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *member_assignment.MemberAssignment_FieldMask
	OrderBy   *member_assignment.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *member_assignment.ParentName
	Filter *member_assignment.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[member_assignment.Name]*member_assignment.MemberAssignment
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*member_assignment.MemberAssignmentChange
}

func NewWatcher(client member_assignment_client.MemberAssignmentServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &member_assignment.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &member_assignment.Filter{}
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[member_assignment.Name]*member_assignment.MemberAssignment{}
...
type WatcherEventChange struct {
	pre, post *member_assignment.MemberAssignment
	name      *member_assignment.Name
}

func NewAddWatcherEventChange(resource *member_assignment.MemberAssignment) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *member_assignment.MemberAssignment) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *member_assignment.MemberAssignment) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *member_assignment.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *member_assignment.MemberAssignment {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *member_assignment.MemberAssignment {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *member_assignment.MemberAssignment {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *member_assignment.MemberAssignment {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[member_assignment.Name]*member_assignment.MemberAssignment, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		memberAssignment := currentChange.GetCurrent()
		byName[*memberAssignment.GetName()] = memberAssignment
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			memberAssignment := change.GetCurrent()
			byName[*memberAssignment.GetName()] = memberAssignment
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, memberAssignment := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(memberAssignment))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[member_assignment.Name]*member_assignment.MemberAssignment{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *member_assignment.MemberAssignment
		switch item := change.ChangeType.(type) {
		case *member_assignment.MemberAssignmentChange_Added_:
			currentItem = item.Added.GetMemberAssignment()
		case *member_assignment.MemberAssignmentChange_Current_:
			currentItem = item.Current.GetMemberAssignment()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*member_assignment.MemberAssignmentChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *member_assignment.MemberAssignmentChange_Added_:
			newItem := item.Added.GetMemberAssignment()
		...
		case *member_assignment.MemberAssignmentChange_Modified_:
			updatedItem := item.Modified.GetMemberAssignment()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*member_assignment.MemberAssignmentChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetMemberAssignmentName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(member_assignment.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*member_assignment.MemberAssignment_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*member_assignment.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(member_assignment_client.NewMemberAssignmentServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(member_assignment.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*member_assignment.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*member_assignment.ParentName)
		}
		return params
	})
}
