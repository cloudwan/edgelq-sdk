...
type QueryWatcherParams struct {
	...
	Cursor       *member_assignment.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*member_assignment.MemberAssignmentChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return member_assignment.MemberAssignmentChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client member_assignment_client.MemberAssignmentServiceClient,
...
func NewQueryWatcherWithIChan(id int, client member_assignment_client.MemberAssignmentServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "memberAssignment-query-watcher").
	...
	for {
		stream, err := qw.client.WatchMemberAssignments(ctx, &member_assignment_client.WatchMemberAssignmentsRequest{
		...
		if err != nil {
			...
			pending := make([]*member_assignment.MemberAssignmentChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetMemberAssignmentChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetMemberAssignmentChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(member_assignment.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(member_assignment.MemberAssignmentChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(member_assignment.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*member_assignment.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*member_assignment.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*member_assignment.Filter)
		}
		return NewQueryWatcherWithIChan(id, member_assignment_client.NewMemberAssignmentServiceClient(cc), cfg, ch)
	})
}
