...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *permission.Permission_FieldMask
	OrderBy   *permission.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *permission.ParentName
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[permission.Name]*permission.Permission
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*permission.PermissionChange
}

func NewWatcher(client permission_client.PermissionServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &permission.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &permission.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "permission-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[permission.Name]*permission.Permission{}
...
type WatcherEventChange struct {
	pre, post *permission.Permission
	name      *permission.Name
}

func NewAddWatcherEventChange(resource *permission.Permission) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *permission.Permission) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *permission.Permission) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *permission.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *permission.Permission {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *permission.Permission {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *permission.Permission {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *permission.Permission {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[permission.Name]*permission.Permission, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		permission := currentChange.GetCurrent()
	...
	for _, change := range src.Changes {
		if change.IsDelete() {
			...
			permission := change.GetCurrent()
			byName[*permission.GetName()] = permission
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, permission := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(permission))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[permission.Name]*permission.Permission{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *permission.Permission
		switch item := change.ChangeType.(type) {
		case *permission.PermissionChange_Added_:
			currentItem = item.Added.GetPermission()
		case *permission.PermissionChange_Current_:
			currentItem = item.Current.GetPermission()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*permission.PermissionChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *permission.PermissionChange_Added_:
			newItem := item.Added.GetPermission()
		...
		case *permission.PermissionChange_Modified_:
			updatedItem := item.Modified.GetPermission()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*permission.PermissionChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetPermissionName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(permission.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*permission.Permission_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*permission.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(permission_client.NewPermissionServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(permission.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*permission.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*permission.ParentName)
		}
		return params
	})
}
