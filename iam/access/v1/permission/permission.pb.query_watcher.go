...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*permission.PermissionChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return permission.PermissionChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client permission_client.PermissionServiceClient,
...
func NewQueryWatcherWithIChan(id int, client permission_client.PermissionServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "permission-query-watcher").
	...
	for {
		stream, err := qw.client.WatchPermissions(ctx, &permission_client.WatchPermissionsRequest{
		...
		if err != nil {
			...
			pending := make([]*permission.PermissionChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetPermissionChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetPermissionChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(permission.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(permission.PermissionChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(permission.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*permission.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*permission.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*permission.Filter)
		}
		return NewQueryWatcherWithIChan(id, permission_client.NewPermissionServiceClient(cc), cfg, ch)
	})
}
