...
func (b *filterCndBuilderInvitationGroups) In(values [][]*group.Reference) *FilterBuilder {
	return b.builder.addCond(&FilterConditionIn{
		OrganizationInvitation_FieldPathArrayOfValues: NewOrganizationInvitationFieldPathBuilder().Invitation().Groups().WithArrayOfValues(values),
	})
}
...
func (b *filterCndBuilderInvitationGroups) IsNull() *FilterBuilder {
	return b.builder.addCond(&FilterConditionIsNull{
		FieldPath: NewOrganizationInvitationFieldPathBuilder().Invitation().Groups().FieldPath(),
	})
}
...
func (b *filterCndBuilderInvitationGroups) Contains(value *group.Reference) *FilterBuilder {
	return b.builder.addCond(&FilterConditionContains{
		Type:      gotenresource.ConditionContainsTypeValue,
		FieldPath: NewOrganizationInvitationFieldPathBuilder().Invitation().Groups().FieldPath(),
		Value:     NewOrganizationInvitationFieldPathBuilder().Invitation().Groups().WithItemValue(value),
	})
}

func (b *filterCndBuilderInvitationGroups) ContainsAnyOf(values []*group.Reference) *FilterBuilder {
	pathSelector := NewOrganizationInvitationFieldPathBuilder().Invitation().Groups()
	itemValues := make([]OrganizationInvitation_FieldPathArrayItemValue, 0, len(values))
	for _, value := range values {
		itemValues = append(itemValues, pathSelector.WithItemValue(value))
	}
...
func (b *filterCndBuilderInvitationGroups) compare(op gotenfilter.CompareOperator, value []*group.Reference) *FilterBuilder {
	return b.builder.addCond(&FilterConditionCompare{
		Operator:                              op,
		OrganizationInvitation_FieldPathValue: NewOrganizationInvitationFieldPathBuilder().Invitation().Groups().WithValue(value),
	})
}
