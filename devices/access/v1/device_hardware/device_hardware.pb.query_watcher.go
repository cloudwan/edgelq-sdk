...
type QueryWatcherParams struct {
	...
	Cursor       *device_hardware.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*device_hardware.DeviceHardwareChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return device_hardware.DeviceHardwareChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client device_hardware_client.DeviceHardwareServiceClient,
...
func NewQueryWatcherWithIChan(id int, client device_hardware_client.DeviceHardwareServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "deviceHardware-query-watcher").
	...
	for {
		stream, err := qw.client.WatchDeviceHardwares(ctx, &device_hardware_client.WatchDeviceHardwaresRequest{
		...
		if err != nil {
			...
			pending := make([]*device_hardware.DeviceHardwareChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetDeviceHardwareChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetDeviceHardwareChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(device_hardware.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(device_hardware.DeviceHardwareChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(device_hardware.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*device_hardware.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*device_hardware.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*device_hardware.Filter)
		}
		return NewQueryWatcherWithIChan(id, device_hardware_client.NewDeviceHardwareServiceClient(cc), cfg, ch)
	})
}
