...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *device_hardware.DeviceHardware_FieldMask
	OrderBy   *device_hardware.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *device_hardware.ParentName
	Filter *device_hardware.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[device_hardware.Name]*device_hardware.DeviceHardware
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*device_hardware.DeviceHardwareChange
}

func NewWatcher(client device_hardware_client.DeviceHardwareServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &device_hardware.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &device_hardware.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "deviceHardware-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[device_hardware.Name]*device_hardware.DeviceHardware{}
...
type WatcherEventChange struct {
	pre, post *device_hardware.DeviceHardware
	name      *device_hardware.Name
}

func NewAddWatcherEventChange(resource *device_hardware.DeviceHardware) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *device_hardware.DeviceHardware) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *device_hardware.DeviceHardware) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *device_hardware.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *device_hardware.DeviceHardware {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *device_hardware.DeviceHardware {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *device_hardware.DeviceHardware {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *device_hardware.DeviceHardware {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[device_hardware.Name]*device_hardware.DeviceHardware, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		deviceHardware := currentChange.GetCurrent()
		byName[*deviceHardware.GetName()] = deviceHardware
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			deviceHardware := change.GetCurrent()
			byName[*deviceHardware.GetName()] = deviceHardware
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, deviceHardware := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(deviceHardware))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[device_hardware.Name]*device_hardware.DeviceHardware{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *device_hardware.DeviceHardware
		switch item := change.ChangeType.(type) {
		case *device_hardware.DeviceHardwareChange_Added_:
			currentItem = item.Added.GetDeviceHardware()
		case *device_hardware.DeviceHardwareChange_Current_:
			currentItem = item.Current.GetDeviceHardware()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*device_hardware.DeviceHardwareChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *device_hardware.DeviceHardwareChange_Added_:
			newItem := item.Added.GetDeviceHardware()
		...
		case *device_hardware.DeviceHardwareChange_Modified_:
			updatedItem := item.Modified.GetDeviceHardware()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*device_hardware.DeviceHardwareChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetDeviceHardwareName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(device_hardware.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*device_hardware.DeviceHardware_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*device_hardware.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(device_hardware_client.NewDeviceHardwareServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(device_hardware.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*device_hardware.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*device_hardware.ParentName)
		}
		return params
	})
}
