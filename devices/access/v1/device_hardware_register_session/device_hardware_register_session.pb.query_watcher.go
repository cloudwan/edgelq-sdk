...
type QueryWatcherParams struct {
	...
	Cursor       *device_hardware_register_session.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*device_hardware_register_session.DeviceHardwareRegisterSessionChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return device_hardware_register_session.DeviceHardwareRegisterSessionChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client device_hardware_register_session_client.DeviceHardwareRegisterSessionServiceClient,
...
func NewQueryWatcherWithIChan(id int, client device_hardware_register_session_client.DeviceHardwareRegisterSessionServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "deviceHardwareRegisterSession-query-watcher").
	...
	for {
		stream, err := qw.client.WatchDeviceHardwareRegisterSessions(ctx, &device_hardware_register_session_client.WatchDeviceHardwareRegisterSessionsRequest{
		...
		if err != nil {
			...
			pending := make([]*device_hardware_register_session.DeviceHardwareRegisterSessionChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetDeviceHardwareRegisterSessionChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetDeviceHardwareRegisterSessionChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(device_hardware_register_session.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(device_hardware_register_session.DeviceHardwareRegisterSessionChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(device_hardware_register_session.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*device_hardware_register_session.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*device_hardware_register_session.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*device_hardware_register_session.Filter)
		}
		return NewQueryWatcherWithIChan(id, device_hardware_register_session_client.NewDeviceHardwareRegisterSessionServiceClient(cc), cfg, ch)
	})
}
