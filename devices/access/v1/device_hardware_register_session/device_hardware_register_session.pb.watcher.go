...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *device_hardware_register_session.DeviceHardwareRegisterSession_FieldMask
	OrderBy   *device_hardware_register_session.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *device_hardware_register_session.ParentName
	Filter *device_hardware_register_session.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[device_hardware_register_session.Name]*device_hardware_register_session.DeviceHardwareRegisterSession
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*device_hardware_register_session.DeviceHardwareRegisterSessionChange
}

func NewWatcher(client device_hardware_register_session_client.DeviceHardwareRegisterSessionServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &device_hardware_register_session.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &device_hardware_register_session.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "deviceHardwareRegisterSession-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[device_hardware_register_session.Name]*device_hardware_register_session.DeviceHardwareRegisterSession{}
...
type WatcherEventChange struct {
	pre, post *device_hardware_register_session.DeviceHardwareRegisterSession
	name      *device_hardware_register_session.Name
}

func NewAddWatcherEventChange(resource *device_hardware_register_session.DeviceHardwareRegisterSession) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *device_hardware_register_session.DeviceHardwareRegisterSession) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *device_hardware_register_session.DeviceHardwareRegisterSession) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *device_hardware_register_session.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *device_hardware_register_session.DeviceHardwareRegisterSession {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *device_hardware_register_session.DeviceHardwareRegisterSession {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *device_hardware_register_session.DeviceHardwareRegisterSession {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *device_hardware_register_session.DeviceHardwareRegisterSession {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[device_hardware_register_session.Name]*device_hardware_register_session.DeviceHardwareRegisterSession, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		deviceHardwareRegisterSession := currentChange.GetCurrent()
		byName[*deviceHardwareRegisterSession.GetName()] = deviceHardwareRegisterSession
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			deviceHardwareRegisterSession := change.GetCurrent()
			byName[*deviceHardwareRegisterSession.GetName()] = deviceHardwareRegisterSession
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, deviceHardwareRegisterSession := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(deviceHardwareRegisterSession))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[device_hardware_register_session.Name]*device_hardware_register_session.DeviceHardwareRegisterSession{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *device_hardware_register_session.DeviceHardwareRegisterSession
		switch item := change.ChangeType.(type) {
		case *device_hardware_register_session.DeviceHardwareRegisterSessionChange_Added_:
			currentItem = item.Added.GetDeviceHardwareRegisterSession()
		case *device_hardware_register_session.DeviceHardwareRegisterSessionChange_Current_:
			currentItem = item.Current.GetDeviceHardwareRegisterSession()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*device_hardware_register_session.DeviceHardwareRegisterSessionChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *device_hardware_register_session.DeviceHardwareRegisterSessionChange_Added_:
			newItem := item.Added.GetDeviceHardwareRegisterSession()
		...
		case *device_hardware_register_session.DeviceHardwareRegisterSessionChange_Modified_:
			updatedItem := item.Modified.GetDeviceHardwareRegisterSession()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*device_hardware_register_session.DeviceHardwareRegisterSessionChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetDeviceHardwareRegisterSessionName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(device_hardware_register_session.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*device_hardware_register_session.DeviceHardwareRegisterSession_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*device_hardware_register_session.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(device_hardware_register_session_client.NewDeviceHardwareRegisterSessionServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(device_hardware_register_session.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*device_hardware_register_session.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*device_hardware_register_session.ParentName)
		}
		return params
	})
}
