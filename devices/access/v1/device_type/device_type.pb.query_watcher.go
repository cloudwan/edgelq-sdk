...
type QueryWatcherParams struct {
	...
	Cursor       *device_type.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*device_type.DeviceTypeChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return device_type.DeviceTypeChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client device_type_client.DeviceTypeServiceClient,
...
func NewQueryWatcherWithIChan(id int, client device_type_client.DeviceTypeServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	...
	for {
		...
		if err != nil {
			...
			pending := make([]*device_type.DeviceTypeChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetDeviceTypeChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetDeviceTypeChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(device_type.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(device_type.DeviceTypeChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(device_type.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		return NewQueryWatcherWithIChan(id, device_type_client.NewDeviceTypeServiceClient(cc), cfg, ch)
	})
}
