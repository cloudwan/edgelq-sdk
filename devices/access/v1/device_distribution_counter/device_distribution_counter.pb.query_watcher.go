...
type QueryWatcherParams struct {
	...
	Cursor       *device_distribution_counter.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*device_distribution_counter.DeviceDistributionCounterChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return device_distribution_counter.DeviceDistributionCounterChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client device_distribution_counter_client.DeviceDistributionCounterServiceClient,
...
func NewQueryWatcherWithIChan(id int, client device_distribution_counter_client.DeviceDistributionCounterServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "deviceDistributionCounter-query-watcher").
	...
	for {
		stream, err := qw.client.WatchDeviceDistributionCounters(ctx, &device_distribution_counter_client.WatchDeviceDistributionCountersRequest{
		...
		if err != nil {
			...
			pending := make([]*device_distribution_counter.DeviceDistributionCounterChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetDeviceDistributionCounterChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetDeviceDistributionCounterChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(device_distribution_counter.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(device_distribution_counter.DeviceDistributionCounterChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(device_distribution_counter.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*device_distribution_counter.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*device_distribution_counter.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*device_distribution_counter.Filter)
		}
		return NewQueryWatcherWithIChan(id, device_distribution_counter_client.NewDeviceDistributionCounterServiceClient(cc), cfg, ch)
	})
}
