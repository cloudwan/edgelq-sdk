...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *device_distribution_counter.DeviceDistributionCounter_FieldMask
	OrderBy   *device_distribution_counter.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *device_distribution_counter.ParentName
	Filter *device_distribution_counter.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[device_distribution_counter.Name]*device_distribution_counter.DeviceDistributionCounter
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*device_distribution_counter.DeviceDistributionCounterChange
}

func NewWatcher(client device_distribution_counter_client.DeviceDistributionCounterServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &device_distribution_counter.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &device_distribution_counter.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "deviceDistributionCounter-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[device_distribution_counter.Name]*device_distribution_counter.DeviceDistributionCounter{}
...
type WatcherEventChange struct {
	pre, post *device_distribution_counter.DeviceDistributionCounter
	name      *device_distribution_counter.Name
}

func NewAddWatcherEventChange(resource *device_distribution_counter.DeviceDistributionCounter) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *device_distribution_counter.DeviceDistributionCounter) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *device_distribution_counter.DeviceDistributionCounter) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *device_distribution_counter.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *device_distribution_counter.DeviceDistributionCounter {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *device_distribution_counter.DeviceDistributionCounter {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *device_distribution_counter.DeviceDistributionCounter {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *device_distribution_counter.DeviceDistributionCounter {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[device_distribution_counter.Name]*device_distribution_counter.DeviceDistributionCounter, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		deviceDistributionCounter := currentChange.GetCurrent()
		byName[*deviceDistributionCounter.GetName()] = deviceDistributionCounter
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			deviceDistributionCounter := change.GetCurrent()
			byName[*deviceDistributionCounter.GetName()] = deviceDistributionCounter
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, deviceDistributionCounter := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(deviceDistributionCounter))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[device_distribution_counter.Name]*device_distribution_counter.DeviceDistributionCounter{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *device_distribution_counter.DeviceDistributionCounter
		switch item := change.ChangeType.(type) {
		case *device_distribution_counter.DeviceDistributionCounterChange_Added_:
			currentItem = item.Added.GetDeviceDistributionCounter()
		case *device_distribution_counter.DeviceDistributionCounterChange_Current_:
			currentItem = item.Current.GetDeviceDistributionCounter()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*device_distribution_counter.DeviceDistributionCounterChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *device_distribution_counter.DeviceDistributionCounterChange_Added_:
			newItem := item.Added.GetDeviceDistributionCounter()
		...
		case *device_distribution_counter.DeviceDistributionCounterChange_Modified_:
			updatedItem := item.Modified.GetDeviceDistributionCounter()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*device_distribution_counter.DeviceDistributionCounterChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetDeviceDistributionCounterName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(device_distribution_counter.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*device_distribution_counter.DeviceDistributionCounter_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*device_distribution_counter.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(device_distribution_counter_client.NewDeviceDistributionCounterServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(device_distribution_counter.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*device_distribution_counter.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*device_distribution_counter.ParentName)
		}
		return params
	})
}
