...
type QueryWatcherParams struct {
	...
	Cursor       *tpm_attestation_cert.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*tpm_attestation_cert.TpmAttestationCertChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return tpm_attestation_cert.TpmAttestationCertChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client tpm_attestation_cert_client.TpmAttestationCertServiceClient,
...
func NewQueryWatcherWithIChan(id int, client tpm_attestation_cert_client.TpmAttestationCertServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "tpmAttestationCert-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchTpmAttestationCerts(ctx, &tpm_attestation_cert_client.WatchTpmAttestationCertsRequest{
		...
		if err != nil {
			...
			pending := make([]*tpm_attestation_cert.TpmAttestationCertChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetTpmAttestationCertChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetTpmAttestationCertChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(tpm_attestation_cert.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(tpm_attestation_cert.TpmAttestationCertChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(tpm_attestation_cert.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*tpm_attestation_cert.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*tpm_attestation_cert.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*tpm_attestation_cert.Filter)
		}
		return NewQueryWatcherWithIChan(id, tpm_attestation_cert_client.NewTpmAttestationCertServiceClient(cc), cfg, ch)
	})
}
