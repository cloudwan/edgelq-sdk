...
type QueryWatcherParams struct {
	...
	Cursor       *provisioning_approval_request.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*provisioning_approval_request.ProvisioningApprovalRequestChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return provisioning_approval_request.ProvisioningApprovalRequestChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client provisioning_approval_request_client.ProvisioningApprovalRequestServiceClient,
...
func NewQueryWatcherWithIChan(id int, client provisioning_approval_request_client.ProvisioningApprovalRequestServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "provisioningApprovalRequest-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchProvisioningApprovalRequests(ctx, &provisioning_approval_request_client.WatchProvisioningApprovalRequestsRequest{
		...
		if err != nil {
			...
			pending := make([]*provisioning_approval_request.ProvisioningApprovalRequestChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetProvisioningApprovalRequestChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetProvisioningApprovalRequestChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(provisioning_approval_request.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(provisioning_approval_request.ProvisioningApprovalRequestChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(provisioning_approval_request.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*provisioning_approval_request.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*provisioning_approval_request.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*provisioning_approval_request.Filter)
		}
		return NewQueryWatcherWithIChan(id, provisioning_approval_request_client.NewProvisioningApprovalRequestServiceClient(cc), cfg, ch)
	})
}
