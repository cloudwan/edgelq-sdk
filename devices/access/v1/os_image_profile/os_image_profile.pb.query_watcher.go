...
type QueryWatcherParams struct {
	...
	Cursor       *os_image_profile.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*os_image_profile.OsImageProfileChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return os_image_profile.OsImageProfileChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client os_image_profile_client.OsImageProfileServiceClient,
...
func NewQueryWatcherWithIChan(id int, client os_image_profile_client.OsImageProfileServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "osImageProfile-query-watcher").
	...
	for {
		stream, err := qw.client.WatchOsImageProfiles(ctx, &os_image_profile_client.WatchOsImageProfilesRequest{
		...
		if err != nil {
			...
			pending := make([]*os_image_profile.OsImageProfileChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetOsImageProfileChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetOsImageProfileChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(os_image_profile.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(os_image_profile.OsImageProfileChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(os_image_profile.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*os_image_profile.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*os_image_profile.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*os_image_profile.Filter)
		}
		return NewQueryWatcherWithIChan(id, os_image_profile_client.NewOsImageProfileServiceClient(cc), cfg, ch)
	})
}
