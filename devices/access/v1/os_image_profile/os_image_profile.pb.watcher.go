...
type WatcherFilterParams struct {
	Parent *os_image_profile.ParentName
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &os_image_profile.ParentName{}
...
func init() {
	...
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(os_image_profile.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*os_image_profile.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*os_image_profile.ParentName)
		}
		return params
	})
}
