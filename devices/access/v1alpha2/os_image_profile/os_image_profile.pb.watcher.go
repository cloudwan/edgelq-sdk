...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *os_image_profile.OsImageProfile_FieldMask
	OrderBy   *os_image_profile.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *os_image_profile.ParentReference
	Filter *os_image_profile.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[os_image_profile.Name]*os_image_profile.OsImageProfile
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*os_image_profile.OsImageProfileChange
}

func NewWatcher(client os_image_profile_client.OsImageProfileServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &os_image_profile.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &os_image_profile.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "osImageProfile-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[os_image_profile.Name]*os_image_profile.OsImageProfile{}
...
type WatcherEventChange struct {
	pre, post *os_image_profile.OsImageProfile
	name      *os_image_profile.Name
}

func NewAddWatcherEventChange(resource *os_image_profile.OsImageProfile) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *os_image_profile.OsImageProfile) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *os_image_profile.OsImageProfile) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *os_image_profile.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *os_image_profile.OsImageProfile {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *os_image_profile.OsImageProfile {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *os_image_profile.OsImageProfile {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *os_image_profile.OsImageProfile {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[os_image_profile.Name]*os_image_profile.OsImageProfile, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		osImageProfile := currentChange.GetCurrent()
		byName[*osImageProfile.GetName()] = osImageProfile
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			osImageProfile := change.GetCurrent()
			byName[*osImageProfile.GetName()] = osImageProfile
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, osImageProfile := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(osImageProfile))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[os_image_profile.Name]*os_image_profile.OsImageProfile{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *os_image_profile.OsImageProfile
		switch item := change.ChangeType.(type) {
		case *os_image_profile.OsImageProfileChange_Added_:
			currentItem = item.Added.GetOsImageProfile()
		case *os_image_profile.OsImageProfileChange_Current_:
			currentItem = item.Current.GetOsImageProfile()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*os_image_profile.OsImageProfileChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *os_image_profile.OsImageProfileChange_Added_:
			newItem := item.Added.GetOsImageProfile()
		...
		case *os_image_profile.OsImageProfileChange_Modified_:
			updatedItem := item.Modified.GetOsImageProfile()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*os_image_profile.OsImageProfileChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetOsImageProfileName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(os_image_profile.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*os_image_profile.OsImageProfile_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*os_image_profile.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(os_image_profile_client.NewOsImageProfileServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(os_image_profile.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*os_image_profile.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*os_image_profile.ParentReference)
		}
		return params
	})
}
