...
type QueryWatcherParams struct {
	...
	Cursor       *customized_image.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*customized_image.CustomizedImageChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return customized_image.CustomizedImageChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client customized_image_client.CustomizedImageServiceClient,
...
func NewQueryWatcherWithIChan(id int, client customized_image_client.CustomizedImageServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "customizedImage-query-watcher").
	...
	for {
		stream, err := qw.client.WatchCustomizedImages(ctx, &customized_image_client.WatchCustomizedImagesRequest{
		...
		if err != nil {
			...
			pending := make([]*customized_image.CustomizedImageChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetCustomizedImageChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetCustomizedImageChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(customized_image.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(customized_image.CustomizedImageChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(customized_image.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*customized_image.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*customized_image.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*customized_image.Filter)
		}
		return NewQueryWatcherWithIChan(id, customized_image_client.NewCustomizedImageServiceClient(cc), cfg, ch)
	})
}
