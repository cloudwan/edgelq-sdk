...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *customized_image.CustomizedImage_FieldMask
	OrderBy   *customized_image.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *customized_image.ParentReference
	Filter *customized_image.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[customized_image.Name]*customized_image.CustomizedImage
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*customized_image.CustomizedImageChange
}

func NewWatcher(client customized_image_client.CustomizedImageServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &customized_image.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &customized_image.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "customizedImage-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[customized_image.Name]*customized_image.CustomizedImage{}
...
type WatcherEventChange struct {
	pre, post *customized_image.CustomizedImage
	name      *customized_image.Name
}

func NewAddWatcherEventChange(resource *customized_image.CustomizedImage) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *customized_image.CustomizedImage) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *customized_image.CustomizedImage) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *customized_image.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *customized_image.CustomizedImage {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *customized_image.CustomizedImage {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *customized_image.CustomizedImage {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *customized_image.CustomizedImage {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[customized_image.Name]*customized_image.CustomizedImage, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		customizedImage := currentChange.GetCurrent()
		byName[*customizedImage.GetName()] = customizedImage
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			customizedImage := change.GetCurrent()
			byName[*customizedImage.GetName()] = customizedImage
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, customizedImage := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(customizedImage))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[customized_image.Name]*customized_image.CustomizedImage{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *customized_image.CustomizedImage
		switch item := change.ChangeType.(type) {
		case *customized_image.CustomizedImageChange_Added_:
			currentItem = item.Added.GetCustomizedImage()
		case *customized_image.CustomizedImageChange_Current_:
			currentItem = item.Current.GetCustomizedImage()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*customized_image.CustomizedImageChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *customized_image.CustomizedImageChange_Added_:
			newItem := item.Added.GetCustomizedImage()
		...
		case *customized_image.CustomizedImageChange_Modified_:
			updatedItem := item.Modified.GetCustomizedImage()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*customized_image.CustomizedImageChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetCustomizedImageName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(customized_image.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*customized_image.CustomizedImage_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*customized_image.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(customized_image_client.NewCustomizedImageServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(customized_image.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*customized_image.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*customized_image.ParentReference)
		}
		return params
	})
}
