...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &os_version.ParentReference{}
...
func init() {
	...
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(os_version.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*os_version.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*os_version.ParentReference)
		}
		return params
	})
}
