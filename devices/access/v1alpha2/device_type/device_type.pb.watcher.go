...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[device_type.Name]*device_type.DeviceType
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*device_type.DeviceTypeChange
}

func NewWatcher(client device_type_client.DeviceTypeServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "deviceType-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[device_type.Name]*device_type.DeviceType{}
...
type WatcherEventChange struct {
	pre, post *device_type.DeviceType
	name      *device_type.Name
}

func NewAddWatcherEventChange(resource *device_type.DeviceType) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *device_type.DeviceType) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *device_type.DeviceType) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *device_type.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *device_type.DeviceType {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *device_type.DeviceType {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *device_type.DeviceType {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *device_type.DeviceType {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[device_type.Name]*device_type.DeviceType, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		deviceType := currentChange.GetCurrent()
		byName[*deviceType.GetName()] = deviceType
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			deviceType := change.GetCurrent()
			byName[*deviceType.GetName()] = deviceType
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, deviceType := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(deviceType))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[device_type.Name]*device_type.DeviceType{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *device_type.DeviceType
		switch item := change.ChangeType.(type) {
		case *device_type.DeviceTypeChange_Added_:
			currentItem = item.Added.GetDeviceType()
		case *device_type.DeviceTypeChange_Current_:
			currentItem = item.Current.GetDeviceType()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*device_type.DeviceTypeChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *device_type.DeviceTypeChange_Added_:
			newItem := item.Added.GetDeviceType()
		...
		case *device_type.DeviceTypeChange_Modified_:
			updatedItem := item.Modified.GetDeviceType()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*device_type.DeviceTypeChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetDeviceTypeName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(device_type.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*device_type.DeviceType_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*device_type.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(device_type_client.NewDeviceTypeServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(device_type.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
