...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	OrderBy   *provisioning_approval_request.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *provisioning_approval_request.ParentReference
	Filter *provisioning_approval_request.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[provisioning_approval_request.Name]*provisioning_approval_request.ProvisioningApprovalRequest
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*provisioning_approval_request.ProvisioningApprovalRequestChange
}

func NewWatcher(client provisioning_approval_request_client.ProvisioningApprovalRequestServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &provisioning_approval_request.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &provisioning_approval_request.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "provisioningApprovalRequest-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[provisioning_approval_request.Name]*provisioning_approval_request.ProvisioningApprovalRequest{}
...
type WatcherEventChange struct {
	pre, post *provisioning_approval_request.ProvisioningApprovalRequest
	name      *provisioning_approval_request.Name
}

func NewAddWatcherEventChange(resource *provisioning_approval_request.ProvisioningApprovalRequest) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *provisioning_approval_request.ProvisioningApprovalRequest) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *provisioning_approval_request.ProvisioningApprovalRequest) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *provisioning_approval_request.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *provisioning_approval_request.ProvisioningApprovalRequest {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *provisioning_approval_request.ProvisioningApprovalRequest {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *provisioning_approval_request.ProvisioningApprovalRequest {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *provisioning_approval_request.ProvisioningApprovalRequest {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[provisioning_approval_request.Name]*provisioning_approval_request.ProvisioningApprovalRequest, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		provisioningApprovalRequest := currentChange.GetCurrent()
		byName[*provisioningApprovalRequest.GetName()] = provisioningApprovalRequest
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			provisioningApprovalRequest := change.GetCurrent()
			byName[*provisioningApprovalRequest.GetName()] = provisioningApprovalRequest
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, provisioningApprovalRequest := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(provisioningApprovalRequest))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[provisioning_approval_request.Name]*provisioning_approval_request.ProvisioningApprovalRequest{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *provisioning_approval_request.ProvisioningApprovalRequest
		switch item := change.ChangeType.(type) {
		case *provisioning_approval_request.ProvisioningApprovalRequestChange_Added_:
			currentItem = item.Added.GetProvisioningApprovalRequest()
		case *provisioning_approval_request.ProvisioningApprovalRequestChange_Current_:
			currentItem = item.Current.GetProvisioningApprovalRequest()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*provisioning_approval_request.ProvisioningApprovalRequestChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *provisioning_approval_request.ProvisioningApprovalRequestChange_Added_:
			newItem := item.Added.GetProvisioningApprovalRequest()
		...
		case *provisioning_approval_request.ProvisioningApprovalRequestChange_Modified_:
			updatedItem := item.Modified.GetProvisioningApprovalRequest()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*provisioning_approval_request.ProvisioningApprovalRequestChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range pendingChanges {
		name := *pendingChange.GetProvisioningApprovalRequestName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(provisioning_approval_request.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*provisioning_approval_request.ProvisioningApprovalRequest_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*provisioning_approval_request.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(provisioning_approval_request_client.NewProvisioningApprovalRequestServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(provisioning_approval_request.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*provisioning_approval_request.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*provisioning_approval_request.ParentReference)
		}
		return params
	})
}
