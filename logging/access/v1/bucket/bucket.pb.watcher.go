...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *bucket.Bucket_FieldMask
	OrderBy   *bucket.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *bucket.ParentName
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[bucket.Name]*bucket.Bucket
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*bucket.BucketChange
}

func NewWatcher(client bucket_client.BucketServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "bucket-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[bucket.Name]*bucket.Bucket{}
...
type WatcherEventChange struct {
	pre, post *bucket.Bucket
	name      *bucket.Name
}

func NewAddWatcherEventChange(resource *bucket.Bucket) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *bucket.Bucket) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *bucket.Bucket) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *bucket.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *bucket.Bucket {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *bucket.Bucket {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *bucket.Bucket {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *bucket.Bucket {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[bucket.Name]*bucket.Bucket, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		bucket := currentChange.GetCurrent()
		byName[*bucket.GetName()] = bucket
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			bucket := change.GetCurrent()
			byName[*bucket.GetName()] = bucket
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, bucket := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(bucket))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*bucket.BucketChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *bucket.BucketChange_Added_:
			newItem := item.Added.GetBucket()
		...
		case *bucket.BucketChange_Modified_:
			updatedItem := item.Modified.GetBucket()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*bucket.BucketChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetBucketName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(bucket.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*bucket.Bucket_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*bucket.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(bucket_client.NewBucketServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(bucket.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*bucket.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*bucket.ParentName)
		}
		return params
	})
}
