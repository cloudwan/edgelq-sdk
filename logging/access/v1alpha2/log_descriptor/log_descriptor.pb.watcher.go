...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	OrderBy   *log_descriptor.OrderBy
	ChunkSize int
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[log_descriptor.Name]*log_descriptor.LogDescriptor
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*log_descriptor.LogDescriptorChange
}
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &log_descriptor.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &log_descriptor.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "logDescriptor-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[log_descriptor.Name]*log_descriptor.LogDescriptor{}
...
type WatcherEventChange struct {
	pre, post *log_descriptor.LogDescriptor
	name      *log_descriptor.Name
}
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[log_descriptor.Name]*log_descriptor.LogDescriptor, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		logDescriptor := currentChange.GetCurrent()
		byName[*logDescriptor.GetName()] = logDescriptor
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			logDescriptor := change.GetCurrent()
			byName[*logDescriptor.GetName()] = logDescriptor
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, logDescriptor := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(logDescriptor))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[log_descriptor.Name]*log_descriptor.LogDescriptor{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *log_descriptor.LogDescriptor
		switch item := change.ChangeType.(type) {
		case *log_descriptor.LogDescriptorChange_Added_:
			currentItem = item.Added.GetLogDescriptor()
		case *log_descriptor.LogDescriptorChange_Current_:
			currentItem = item.Current.GetLogDescriptor()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*log_descriptor.LogDescriptorChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *log_descriptor.LogDescriptorChange_Added_:
			newItem := item.Added.GetLogDescriptor()
		...
		case *log_descriptor.LogDescriptorChange_Modified_:
			updatedItem := item.Modified.GetLogDescriptor()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*log_descriptor.LogDescriptorChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetLogDescriptorName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(log_descriptor.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*log_descriptor.LogDescriptor_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*log_descriptor.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(log_descriptor_client.NewLogDescriptorServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(log_descriptor.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*log_descriptor.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*log_descriptor.ParentReference)
		}
		return params
	})
}
