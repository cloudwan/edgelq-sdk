...
type QueryWatcherParams struct {
	...
	Cursor       *log_descriptor.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*log_descriptor.LogDescriptorChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return log_descriptor.LogDescriptorChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client log_descriptor_client.LogDescriptorServiceClient,
...
func NewQueryWatcherWithIChan(id int, client log_descriptor_client.LogDescriptorServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "logDescriptor-query-watcher").
	...
	for {
		stream, err := qw.client.WatchLogDescriptors(ctx, &log_descriptor_client.WatchLogDescriptorsRequest{
		...
		if err != nil {
			...
			pending := make([]*log_descriptor.LogDescriptorChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetLogDescriptorChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetLogDescriptorChanges(),
...
func init() {
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(log_descriptor.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		return NewQueryWatcherWithIChan(id, log_descriptor_client.NewLogDescriptorServiceClient(cc), cfg, ch)
	})
}
