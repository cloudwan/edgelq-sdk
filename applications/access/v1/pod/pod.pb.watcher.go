...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[pod.Name]*pod.Pod
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*pod.PodChange
}

func NewWatcher(client pod_client.PodServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[pod.Name]*pod.Pod{}
...
func NewAddWatcherEventChange(resource *pod.Pod) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *pod.Pod) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *pod.Pod) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *pod.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *pod.Pod {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *pod.Pod {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *pod.Pod {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *pod.Pod {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[pod.Name]*pod.Pod, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		pod := currentChange.GetCurrent()
		byName[*pod.GetName()] = pod
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			pod := change.GetCurrent()
			byName[*pod.GetName()] = pod
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, pod := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(pod))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*pod.PodChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *pod.PodChange_Added_:
			newItem := item.Added.GetPod()
		...
		case *pod.PodChange_Modified_:
			updatedItem := item.Modified.GetPod()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*pod.PodChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetPodName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(pod.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*pod.Pod_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*pod.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(pod_client.NewPodServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(pod.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*pod.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*pod.ParentName)
		}
		return params
	})
}
