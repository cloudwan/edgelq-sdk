...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &config_map.ParentName{}
...
func init() {
	...
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(config_map.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*config_map.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*config_map.ParentName)
		}
		return params
	})
}
