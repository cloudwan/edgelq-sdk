...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*config_map.ConfigMapChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return config_map.ConfigMapChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client config_map_client.ConfigMapServiceClient,
...
func NewQueryWatcherWithIChan(id int, client config_map_client.ConfigMapServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("query-watcher", "configMap-query-watcher").
	...
	inSync := false
	for {
		stream, err := qw.client.WatchConfigMaps(ctx, &config_map_client.WatchConfigMapsRequest{
		...
		if err != nil {
			...
			pending := make([]*config_map.ConfigMapChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetConfigMapChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetConfigMapChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(config_map.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(config_map.ConfigMapChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(config_map.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*config_map.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*config_map.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*config_map.Filter)
		}
		return NewQueryWatcherWithIChan(id, config_map_client.NewConfigMapServiceClient(cc), cfg, ch)
	})
}
