...
type WatcherFilterParams struct {
	Parent *distribution.ParentName
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[distribution.Name]*distribution.Distribution
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*distribution.DistributionChange
}

func NewWatcher(client distribution_client.DistributionServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &distribution.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &distribution.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "distribution-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[distribution.Name]*distribution.Distribution{}
...
type WatcherEventChange struct {
	pre, post *distribution.Distribution
	name      *distribution.Name
}

func NewAddWatcherEventChange(resource *distribution.Distribution) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *distribution.Distribution) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *distribution.Distribution) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *distribution.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *distribution.Distribution {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *distribution.Distribution {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *distribution.Distribution {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *distribution.Distribution {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[distribution.Name]*distribution.Distribution, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		distribution := currentChange.GetCurrent()
		byName[*distribution.GetName()] = distribution
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			distribution := change.GetCurrent()
			byName[*distribution.GetName()] = distribution
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, distribution := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(distribution))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[distribution.Name]*distribution.Distribution{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *distribution.Distribution
		switch item := change.ChangeType.(type) {
		case *distribution.DistributionChange_Added_:
			currentItem = item.Added.GetDistribution()
		case *distribution.DistributionChange_Current_:
			currentItem = item.Current.GetDistribution()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*distribution.DistributionChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *distribution.DistributionChange_Added_:
			newItem := item.Added.GetDistribution()
		...
		case *distribution.DistributionChange_Modified_:
			updatedItem := item.Modified.GetDistribution()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*distribution.DistributionChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetDistributionName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(distribution.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*distribution.Distribution_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*distribution.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(distribution_client.NewDistributionServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(distribution.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*distribution.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*distribution.ParentName)
		}
		return params
	})
}
