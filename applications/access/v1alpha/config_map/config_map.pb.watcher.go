...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *config_map.ConfigMap_FieldMask
	OrderBy   *config_map.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *config_map.ParentReference
	Filter *config_map.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[config_map.Name]*config_map.ConfigMap
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*config_map.ConfigMapChange
}

func NewWatcher(client config_map_client.ConfigMapServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &config_map.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &config_map.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "configMap-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[config_map.Name]*config_map.ConfigMap{}
...
type WatcherEventChange struct {
	pre, post *config_map.ConfigMap
	name      *config_map.Name
}

func NewAddWatcherEventChange(resource *config_map.ConfigMap) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *config_map.ConfigMap) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *config_map.ConfigMap) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *config_map.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *config_map.ConfigMap {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *config_map.ConfigMap {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *config_map.ConfigMap {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *config_map.ConfigMap {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[config_map.Name]*config_map.ConfigMap, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		configMap := currentChange.GetCurrent()
		byName[*configMap.GetName()] = configMap
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			configMap := change.GetCurrent()
			byName[*configMap.GetName()] = configMap
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, configMap := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(configMap))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[config_map.Name]*config_map.ConfigMap{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *config_map.ConfigMap
		switch item := change.ChangeType.(type) {
		case *config_map.ConfigMapChange_Added_:
			currentItem = item.Added.GetConfigMap()
		case *config_map.ConfigMapChange_Current_:
			currentItem = item.Current.GetConfigMap()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*config_map.ConfigMapChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *config_map.ConfigMapChange_Added_:
			newItem := item.Added.GetConfigMap()
		...
		case *config_map.ConfigMapChange_Modified_:
			updatedItem := item.Modified.GetConfigMap()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*config_map.ConfigMapChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetConfigMapName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(config_map.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*config_map.ConfigMap_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*config_map.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(config_map_client.NewConfigMapServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(config_map.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*config_map.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*config_map.ParentReference)
		}
		return params
	})
}
