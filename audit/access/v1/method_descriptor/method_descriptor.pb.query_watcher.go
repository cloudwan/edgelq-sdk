...
type QueryWatcherParams struct {
	...
	Cursor       *method_descriptor.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*method_descriptor.MethodDescriptorChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return method_descriptor.MethodDescriptorChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client method_descriptor_client.MethodDescriptorServiceClient,
...
func NewQueryWatcherWithIChan(id int, client method_descriptor_client.MethodDescriptorServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "methodDescriptor-query-watcher").
	...
	for {
		stream, err := qw.client.WatchMethodDescriptors(ctx, &method_descriptor_client.WatchMethodDescriptorsRequest{
		...
		if err != nil {
			...
			pending := make([]*method_descriptor.MethodDescriptorChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetMethodDescriptorChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetMethodDescriptorChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherEventConstructor(method_descriptor.GetDescriptor(),
		func(evtId int, changes gotenresource.ResourceChangeList, isReset, isLostSync, isCurrent bool, snapshotSize int64) gotenaccess.QueryWatcherEvent {
			return &QueryWatcherEvent{
				Identifier:   evtId,
				Changes:      changes.(method_descriptor.MethodDescriptorChangeList),
				Reset:        isReset,
				LostSync:     isLostSync,
				InSync:       isCurrent,
	...
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(method_descriptor.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*method_descriptor.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*method_descriptor.ParentName)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*method_descriptor.Filter)
		}
		return NewQueryWatcherWithIChan(id, method_descriptor_client.NewMethodDescriptorServiceClient(cc), cfg, ch)
	})
}
