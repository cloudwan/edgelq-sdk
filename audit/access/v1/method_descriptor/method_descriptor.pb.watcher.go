...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *method_descriptor.MethodDescriptor_FieldMask
	OrderBy   *method_descriptor.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *method_descriptor.ParentName
	Filter *method_descriptor.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[method_descriptor.Name]*method_descriptor.MethodDescriptor
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*method_descriptor.MethodDescriptorChange
}

func NewWatcher(client method_descriptor_client.MethodDescriptorServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &method_descriptor.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &method_descriptor.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "methodDescriptor-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[method_descriptor.Name]*method_descriptor.MethodDescriptor{}
...
type WatcherEventChange struct {
	pre, post *method_descriptor.MethodDescriptor
	name      *method_descriptor.Name
}

func NewAddWatcherEventChange(resource *method_descriptor.MethodDescriptor) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *method_descriptor.MethodDescriptor) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *method_descriptor.MethodDescriptor) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *method_descriptor.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *method_descriptor.MethodDescriptor {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *method_descriptor.MethodDescriptor {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *method_descriptor.MethodDescriptor {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *method_descriptor.MethodDescriptor {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[method_descriptor.Name]*method_descriptor.MethodDescriptor, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		methodDescriptor := currentChange.GetCurrent()
		byName[*methodDescriptor.GetName()] = methodDescriptor
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			methodDescriptor := change.GetCurrent()
			byName[*methodDescriptor.GetName()] = methodDescriptor
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, methodDescriptor := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(methodDescriptor))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[method_descriptor.Name]*method_descriptor.MethodDescriptor{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *method_descriptor.MethodDescriptor
		switch item := change.ChangeType.(type) {
		case *method_descriptor.MethodDescriptorChange_Added_:
			currentItem = item.Added.GetMethodDescriptor()
		case *method_descriptor.MethodDescriptorChange_Current_:
			currentItem = item.Current.GetMethodDescriptor()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*method_descriptor.MethodDescriptorChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *method_descriptor.MethodDescriptorChange_Added_:
			newItem := item.Added.GetMethodDescriptor()
		...
		case *method_descriptor.MethodDescriptorChange_Modified_:
			updatedItem := item.Modified.GetMethodDescriptor()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*method_descriptor.MethodDescriptorChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetMethodDescriptorName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(method_descriptor.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*method_descriptor.MethodDescriptor_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*method_descriptor.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
	...
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(method_descriptor.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		...
		if parent != nil {
			params.Parent = parent.(*method_descriptor.ParentName)
		}
		return params
	})
}
