...
type WatcherFilterParams struct {
	Parent *audited_resource_descriptor.ParentName
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[audited_resource_descriptor.Name]*audited_resource_descriptor.AuditedResourceDescriptor
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*audited_resource_descriptor.AuditedResourceDescriptorChange
}

func NewWatcher(client audited_resource_descriptor_client.AuditedResourceDescriptorServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &audited_resource_descriptor.ParentName{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &audited_resource_descriptor.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := gotenobservability.LoggerFromContext(ctx).
		WithField("watcher", "auditedResourceDescriptor-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[audited_resource_descriptor.Name]*audited_resource_descriptor.AuditedResourceDescriptor{}
...
type WatcherEventChange struct {
	pre, post *audited_resource_descriptor.AuditedResourceDescriptor
	name      *audited_resource_descriptor.Name
}

func NewAddWatcherEventChange(resource *audited_resource_descriptor.AuditedResourceDescriptor) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *audited_resource_descriptor.AuditedResourceDescriptor) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *audited_resource_descriptor.AuditedResourceDescriptor) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *audited_resource_descriptor.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *audited_resource_descriptor.AuditedResourceDescriptor {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *audited_resource_descriptor.AuditedResourceDescriptor {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *audited_resource_descriptor.AuditedResourceDescriptor {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *audited_resource_descriptor.AuditedResourceDescriptor {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[audited_resource_descriptor.Name]*audited_resource_descriptor.AuditedResourceDescriptor, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		auditedResourceDescriptor := currentChange.GetCurrent()
		byName[*auditedResourceDescriptor.GetName()] = auditedResourceDescriptor
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			auditedResourceDescriptor := change.GetCurrent()
			byName[*auditedResourceDescriptor.GetName()] = auditedResourceDescriptor
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, auditedResourceDescriptor := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(auditedResourceDescriptor))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[audited_resource_descriptor.Name]*audited_resource_descriptor.AuditedResourceDescriptor{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *audited_resource_descriptor.AuditedResourceDescriptor
		switch item := change.ChangeType.(type) {
		case *audited_resource_descriptor.AuditedResourceDescriptorChange_Added_:
			currentItem = item.Added.GetAuditedResourceDescriptor()
		case *audited_resource_descriptor.AuditedResourceDescriptorChange_Current_:
			currentItem = item.Current.GetAuditedResourceDescriptor()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*audited_resource_descriptor.AuditedResourceDescriptorChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *audited_resource_descriptor.AuditedResourceDescriptorChange_Added_:
			newItem := item.Added.GetAuditedResourceDescriptor()
		...
		case *audited_resource_descriptor.AuditedResourceDescriptorChange_Modified_:
			updatedItem := item.Modified.GetAuditedResourceDescriptor()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(audited_resource_descriptor.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		return NewWatcher(audited_resource_descriptor_client.NewAuditedResourceDescriptorServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(audited_resource_descriptor.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Name) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*audited_resource_descriptor.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*audited_resource_descriptor.ParentName)
		}
		return params
	})
}
